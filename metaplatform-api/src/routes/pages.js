...
router.get("/:id", async (req, res, next) => {
  try {
    const row = await db.prepare("SELECT * FROM app_pages WHERE id = ?").get(req.params.id);
...
router.post("/", async (req, res, next) => {
  try {
    ...
    // Verify app exists
    const app = await db.prepare("SELECT id FROM applications WHERE id = ?").get(app_id);
...
router.put("/:id", async (req, res, next) => {
  try {
    const existing = await db.prepare("SELECT * FROM app_pages WHERE id = ?").get(req.params.id);
...
router.delete("/:id", async (req, res, next) => {
  try {
    const existing = await db.prepare("SELECT * FROM app_pages WHERE id = ?").get(req.params.id);
    if (!existing) return res.status(404).json({ success: false, error: "页面不存在" });
    await db.prepare("DELETE FROM app_pages WHERE id = ?").run(req.params.id);
