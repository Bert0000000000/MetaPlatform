...
router.get("/instances", async (req, res, next) => {
  try {
    const rows = await db.prepare("SELECT * FROM process_instances ORDER BY started_at DESC").all();
    res.json({ success: true, data: rows });
  } catch (err) {
    next(err);
  }
...
router.get("/:id", async (req, res, next) => {
  try {
    const row = await db.prepare("SELECT * FROM process_definitions WHERE id = ?").get(req.params.id);
...
router.post("/", async (req, res, next) => {
  try {
    ...
    const row = await db.prepare("SELECT * FROM process_definitions WHERE id = ?").get(id);
...
router.put("/:id", async (req, res, next) => {
  try {
    const existing = await db.prepare("SELECT * FROM process_definitions WHERE id = ?").get(req.params.id);
...
router.delete("/:id", async (req, res, next) => {
  try {
    const existing = await db.prepare("SELECT * FROM process_definitions WHERE id = ?").get(req.params.id);
    if (!existing) return res.status(404).json({ success: false, error: "流程定义不存在" });
    await db.prepare("DELETE FROM process_definitions WHERE id = ?").run(req.params.id);
...
router.post("/instances", async (req, res, next) => {
  try {
    ...
    const def = await db.prepare("SELECT * FROM process_definitions WHERE id = ?").get(definition_id);
    ...
    const row = await db.prepare("SELECT * FROM process_instances WHERE id = ?").get(id);
    res.status(201).json({ success: true, data: row });
  } catch (err) {
    next(err);
  }
...
router.put("/instances/:id", async (req, res, next) => {
  try {
    const existing = await db.prepare("SELECT * FROM process_instances WHERE id = ?").get(req.params.id);
    ...
    const row = await db.prepare("SELECT * FROM process_instances WHERE id = ?").get(req.params.id);
    res.json({ success: true, data: row });
  } catch (err) {
    next(err);
  }
...
router.post("/instances/:id/intervene", async (req, res, next) => {
  try {
    const instance = await db.prepare("SELECT * FROM process_instances WHERE id = ?").get(req.params.id);
    ...
    const now = new Date().toISOString();
    await db.prepare("UPDATE process_instances SET status = ?, ended_at = ? WHERE id = ?")
