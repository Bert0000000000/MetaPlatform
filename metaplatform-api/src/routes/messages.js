...
router.get("/", async (req, res, next) => {
  try {
    ...
    const rows = await db.prepare("SELECT * FROM messages WHERE user_id = ? ORDER BY created_at DESC").all(userId);
...
router.get("/unread-count", async (req, res, next) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ success: false, error: "未登录" });
    const row = await db.prepare("SELECT COUNT(*) AS cnt FROM messages WHERE user_id = ? AND read = 0").get(userId);
...
router.put("/:id/read", async (req, res, next) => {
  try {
    ...
    await db.prepare("UPDATE messages SET read = 1 WHERE id = ?").run(req.params.id);
    const row = await db.prepare("SELECT * FROM messages WHERE id = ?").get(req.params.id);
