...
router.post("/", async (req, res, next) => {
  try {
    ...
    const { name, scopes = "read", appId = null, expiresAt = null } = req.body || {};
    ...
    db.prepare(
      `INSERT INTO api_keys (id, tenant_id, name, key_hash, key_prefix, scopes, app_id, created_by, expires_at, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(id, tenantId, name, hash, prefix, scopes, appId, userId, expiresAt, now);
