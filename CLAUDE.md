...
│   ├── 005-RD/                # 调研报告
│   ├── 006-TMP/               # 临时脚本
│   └── superpowers/            # spec + plan（subagent-driven execution）
│
├── metaplatform-frontend/     # 🆕 pnpm monorepo（v1.2 阶段 0-1 引入）
│   ├── pnpm-workspace.yaml    #   workspaces: apps/*, packages/*
│   ├── package.json           #   root: 调度 dev/build/test
│   ├── apps/                  #   7 个 APP-* 子应用（按模块）
│   │   ├── portal/            #   登录壳 + 路由聚合（待建）
│   │   ├── ontstudio/         #   APP-ONTSTUDIO 搬入（已建）
│   │   ├── dashboard/         #   占位（待 APP-DASHBOARD 迁入）
│   │   ├── superai/           #   占位
│   │   ├── apphub/            #   占位
│   │   ├── dw/                #   占位
│   │   ├── arch/              #   占位
│   │   └── mcphub/            #   占位
│   └── packages/              #   共享包
│       └── shared/            #   @mate/shared（错误边界、主题壳、AppLayout）
│
├── APP-ONTSTUDIO/             # ⚠️ 待阶段 4 末尾 git rm（已迁入 apps/ontstudio/）
├── APP-DASHBOARD/             # 仪表盘（待 monorepo 迁入）
├── APP-SUPERAI/               # 超级 AI（待 monorepo 迁入）
├── APP-DW/                    # 数字员工（待 monorepo 迁入）
├── APP-APPHUB/                # 应用中心（待 monorepo 迁入）
├── APP-ARCH/                  # 架构中心（待 monorepo 迁入）
├── APP-MCPHUB/                # MCP 服务中心（待 monorepo 迁入）
│
...
> - 阶段 0（2-3 周）：**monorepo 基础 + IAM→ONT→RULE 底层链路** ✅ 部分完成
> - 阶段 1（3-4 周）：**6 服务骨架 + Nacos**
> - 阶段 2（4-6 周）：**核心服务 Java + SAA 重写**（含 TECH-ONT 收敛）
> - 阶段 3（3-4 周）：**MCP / A2A 协议层**
> - 阶段 4（2-3 周）：**生产化与 Python 下线**
>
> **ONT 模块迁移执行**（首个示范模块）：
> - spec：`docs/superpowers/specs/2026-07-21-ont-module-monorepo-v12-design.md`
> - plan：`docs/superpowers/plans/2026-07-21-ont-module-monorepo-v12.md`
>
