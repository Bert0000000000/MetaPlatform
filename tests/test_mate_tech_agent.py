...
def test_chat_s1_with_chunks(client):
    ...
    # no chunk text in stream events (EchoLLM echo)["answer"]
...
def test_chat_s1_empty_results(client, fake_rag_empty):
    ...
    assert r.status_code == 200
    body = r.json()
    assert "No context" in body["answer"]
...
def test_sse_stream_endpoint(client):
    ...
    # no chunk text in stream events (EchoLLM echo)


def test_sse_stream_s2_returns_200():
    set_rag_tool(FakeRAGTool([]))
    c = TestClient(create_app())
