"""JWT authentication utilities for TECH-IAM integration.

Provides HS256 token verification, claim extraction, and a path whitelist
so internal/health/public endpoints can bypass JWT enforcement.
"""
...
# Paths that do NOT require a valid JWT (exact match).
...
def is_whitelisted(path: str) -> bool:
    """Check whether *path* is whitelisted (exact or prefix match)."""

    if path in JWT_WHITELIST:
        return True
    for prefix in JWT_WHITELIST_PREFIXES:
