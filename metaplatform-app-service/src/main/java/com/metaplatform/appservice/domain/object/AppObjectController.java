...
public class AppObjectController {
    ...
    @GetMapping
    public ApiResponse<List<AppObjectEntity>> list(@PathVariable Long appId) {
        return ApiResponse.ok(service.list(appId), MDC.get("traceId"));
    }

    @GetMapping("/{oid}")
    public ApiResponse<AppObjectEntity> get(@PathVariable Long appId,
                                            @PathVariable Long oid) {
        return ApiResponse.ok(service.get(appId, oid), MDC.get("traceId"));
    }

    @PostMapping
    public ApiResponse<AppObjectEntity> create(@PathVariable Long appId,
                                               @RequestBody AppObjectService.AppObjectCreateRequest req) {
        return ApiResponse.ok(service.create(appId, req), MDC.get("traceId"));
    }

    @PutMapping("/{oid}")
    public ApiResponse<AppObjectEntity> update(@PathVariable Long appId,
                                               @PathVariable Long oid,
    ...
    @DeleteMapping("/{oid}")
    public ApiResponse<Map<String, Object>> delete(@PathVariable Long appId,
                                                   @PathVariable Long oid) {
