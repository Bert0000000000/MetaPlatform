...
class TestModelServiceSync:
    async def test_sync_models_success(self, registry):
        # Service-level: full catalog sync writes per-tenant + public rows.
        result = await registry.model_service.sync(TENANT)
        ...
        public_models = [
            m for m in await registry.model_repo.all() if m.tenant_id == PUBLIC_TENANT_ID
        ]
        assert len(public_models) == len(ModelCatalog.CATALOG)

    async def test_sync_models_filters_inactive_provider(self, registry):
        # Only sync OPENAI -> only OPENAI rows appear for the tenant.
        result = await registry.model_service.sync(TENANT, providers=["OPENAI"])
        tenant_models = await registry.model_service.list(TENANT)
        providers = {m.provider for m in tenant_models}
        assert providers == {"OPENAI"}
        assert result["providers"][0]["fetched"] > 0

    async def test_sync_unknown_provider_raises(self, registry):
        with pytest.raises(ProviderNotFoundError):
            await registry.model_service.sync(TENANT, providers=["NOT_A_PROVIDER"])


class TestModelServiceList:
    async def test_list_filters_by_provider_type_enabled(self, seeded_models):
        # Provider filter
        openai = await seeded_models.model_service.list(TENANT, provider="OPENAI")
        assert all(m.provider == "OPENAI" for m in openai)
        # Type filter
        mm = await seeded_models.model_service.list(TENANT, type_=ModelType.MULTIMODAL)
        assert all(m.type == ModelType.MULTIMODAL for m in mm)
        # Disabled filter
        await seeded_models.model_service.list(TENANT, enabled=False)
        # No filter (all enabled)
        all_models = await seeded_models.model_service.list(TENANT)
        assert len(all_models) == len(ModelCatalog.CATALOG)

    async def test_global_models_includes_public_and_tenant(self, seeded_models):
        # Tenant-only first.
        tenant_only = await seeded_models.model_service.list(TENANT)
        ...
        global_models = await seeded_models.model_service.list_global(TENANT)
    ...
    async def test_tenant_isolation_blocks_cross_tenant_model(self, seeded_models):
        ...
        assert await seeded_models.model_service.list(other_tenant) == []
        # Now sync for them and verify the rows are theirs only.
        await seeded_models.model_service.sync(other_tenant)
        others = await seeded_models.model_service.list(other_tenant)
        assert all(m.tenant_id == other_tenant for m in others)
        # And the test tenant's list is independent of the other tenant.
        tenant_models = await seeded_models.model_service.list(TENANT)
...
class TestModelServiceDetail:
    async def test_get_model_detail_404_when_missing(self, registry):
        with pytest.raises(ModelNotFoundError):
            await registry.model_service.detail(TENANT, "m-does-not-exist")
