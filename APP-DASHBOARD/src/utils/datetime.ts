 ...
 * Falls back to browser default locale/timezone when settings are absent.
 */
export function formatDateTime(
  date: string | Date | null | undefined,
  settings?: Pick<UserSettings, 'language' | 'timezone' | 'dateFormat'>,
): string {
  if (!date) return '-';
...
export function formatDate(
  date: string | Date | null | undefined,
  settings?: Pick<UserSettings, 'language' | 'timezone'>,
): string {
  if (!date) return '-';
  const d = typeof date === 'string' ? new Date(date) : date;
  if (Number.isNaN(d.getTime())) return '-';
