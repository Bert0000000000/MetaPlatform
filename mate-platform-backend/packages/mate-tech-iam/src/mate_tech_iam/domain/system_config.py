...
class SystemConfig(SQLModel, table=True):
    ...
    value: str | None = Field(
        default=None, sa_column=Column(Text), description="配置值 (JSON 序列化)"
    )
    ...
    value_type: str = Field(
        default="string", max_length=32, description="string/int/bool/json/enum"
    )
    enum_options: str | None = Field(
        default=None, max_length=512, description="可选值，逗号分隔 (当 value_type=enum)"
    )
