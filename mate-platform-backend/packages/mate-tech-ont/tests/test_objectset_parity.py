...
def _seed(repo):
    ...
    for i, (cat, qty) in enumerate([("a", 10), ("a", 20), ("b", 5), ("c", 7)]):
        ind = Individual(
            rid=f"ont.{T}.ind.item.p{i}",
            class_rid=ClassRef(OBJ),
            props=((ClassRef(P1), cat), (ClassRef(P2), qty)),
            primary_key=f"p{i}",
            tenant_id=T,
            created_at=_dt.now(_U),
            updated_at=_dt.now(_U),
        )
    ...
    repo.create_link_instance(
        LinkInstance(
            rid=f"ont.{T}.lnk.item-rel.{ids[0][-2:]}.{ids[2][-2:]}",
...
def _inmemory(repo, q):
    ...
    conn = psycopg2.connect(PG_DSN)
    with conn.cursor() as cur:
        cur.execute("SELECT rid, src, dst FROM ont_link_instance WHERE link_type_rid=%s", (LINK,))
        rows = cur.fetchall()
    conn.close()
    links = [
        LinkInstance(
...
class TestParity:
    def test_filter_parity(self, repo) -> None:
        ...
        assert (
            sorted(r["item-cat"] for r in pg.rows)
            == sorted(r["item-cat"] for r in mem.rows)
            == ["a", "a"]
        )
    ...
    def test_aggregate_parity(self, repo) -> None:
        ...
        q = ObjectSetQuery(
            ...
            aggregation=Aggregation(
                group_by=("item-cat",),
                metrics=(
                    MetricSpec(fn="sum", field="item-qty", alias="total"),
        ...
        assert key(pg.rows) == key(mem.rows) == {"a": (30, 2), "b": (5, 1), "c": (7, 1)}
    ...
    def test_traversal_parity(self, repo) -> None:
        ...
        assert (
            sorted(r.get("item-cat") for r in pg.rows)
            == sorted(r.get("item-cat") for r in mem.rows)
            == ["b"]
        )
