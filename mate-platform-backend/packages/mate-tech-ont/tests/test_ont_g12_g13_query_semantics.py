...
def _repo() -> InMemoryOntologyRepository:
    ...
    r.create_individual(
        Individual(
            rid=f"ont.{T}.ind.tagged-doc.d1",
            class_rid=ClassRef(OBJ),
            props=(
                (ClassRef(P_ID), "d1"),
                (ClassRef(P_TAGS), ["urgent", "beta"]),
...
class TestNearestIR:
    def test_nearest_inmemory(self) -> None:
        r = _repo()
        result = r.execute_object_query(
            ObjectSetQuery(source=OBJ, nearest=NearestSpec(text="contract final", k=1))
        )
...
PG_DSN = os.environ.get("G12G13_PG_DSN", "postgresql://meta:meta@127.0.0.1:5432/metaplatform_ont")
