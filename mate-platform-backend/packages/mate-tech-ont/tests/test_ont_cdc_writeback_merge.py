...
T = "cdctest"
PG_DSN = os.environ.get("CDC_PG_DSN", "postgresql://meta:meta@127.0.0.1:5432/metaplatform_ont")
...
class TestDualStreamMerge:
    def test_user_edit_surveys_resync(self, repo) -> None:
        with repo.tenant_scope(T):
            ...
            repo.apply_edit_set_now(
                ACT,
                f"ont.{T}.ind.account.a1",
                {},
                [
                    {
                        "op": "set_property",
                        "target": f"ont.{T}.ind.account.a1",
                        "property_rid": P_NAME,
                        "value": "user-edited-name",
                    }
                ],
                actor="sales-1",
            )
    ...
    def test_cdc_entry_upsert_delete(self, repo) -> None:
        with repo.tenant_scope(T):
            ...
            repo.apply_edit_set_now(
                ACT,
                f"ont.{T}.ind.account.a9",
                {},
                [
                    {
                        "op": "set_property",
                        "target": f"ont.{T}.ind.account.a9",
                        "property_rid": P_NAME,
                        "value": "keep-mine",
                    }
                ],
