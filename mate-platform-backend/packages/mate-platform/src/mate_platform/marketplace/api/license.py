...
async def activate_license(body: dict, request: Request):
    ...
    if user is None or "platform.marketplace.write" not in getattr(user, "scopes", frozenset()):
        raise HTTPException(
            status_code=403,
            detail={
...
async def list_subscriptions(request: Request):
    ...
    rows = (
        await session.scalars(
            select(Subscription).where(Subscription.tenant_id == UUID(user.tenant_id))
        )
    ).all()
    return {
        "items": [
