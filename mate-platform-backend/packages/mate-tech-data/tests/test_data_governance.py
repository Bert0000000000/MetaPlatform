...
def test_lineage_edge_registration_and_subgraph():
    _reset()
    repo.create_lineage_edge("tenant-default",
                             "ont.x.obj.orders.v1", "ont.x.obj.orders-ads.v1")
    repo.create_lineage_edge("tenant-default",
                             "ont.x.obj.orders-ads.v1", "ont.x.obj.report.v1")
    graph = repo.lineage_graph("tenant-default", "ont.x.obj.orders-ads.v1")
    node_ids = {n["id"] for n in graph["nodes"]}
    assert node_ids == {"ont.x.obj.orders-ads.v1", "ont.x.obj.orders.v1",
                        "ont.x.obj.report.v1"}
...
def test_quality_rule_required_pass():
    _reset()
    repo.create_quality_rule("tenant-default", "src-mysql-orders", "amount",
...
def test_quality_rule_type_mismatch_fails():
    _reset()
    repo.create_quality_rule("tenant-default", "src-mysql-orders", "amount",
                             "type", {"table": "orders", "type": "varchar(8)"})
    summary = repo.run_quality_rules("tenant-default")
...
def test_quality_missing_column_fails():
    _reset()
    repo.create_quality_rule("tenant-default", "src-mysql-orders", "ghost_col",
                             "required", {"table": "orders"})
    summary = repo.run_quality_rules("tenant-default")
    assert summary["failed"] == 1
