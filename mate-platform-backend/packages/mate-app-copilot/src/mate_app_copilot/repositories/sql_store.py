...
def seed_from_inmemory(tenant_id: str) -> dict[str, int]:
    ...
    counts["conversations"] = len(
        [put_conversation(tenant_id, c) for c in mem.list_conversations(tenant_id)]
    )
    ...
    counts["datasources"] = len(
        [put_datasource(tenant_id, d) for d in mem.list_datasources(tenant_id)]
    )
    counts["knowledge_bases"] = len(
        [put_knowledge_base(tenant_id, k) for k in mem.list_knowledge_bases(tenant_id)]
    )
