"""Cross-tenant integration tests for mate-tech-scheduler (ADR-0014 step 5).
...
Tests:
  ...
  - test_tenant_isolation_tasks: two tenants see disjoint scheduler task catalogs.
  - test_tenant_isolation_dag: two tenants see disjoint DAG graphs.
...
from mate_tech_scheduler.main import create_app
from mate_tech_scheduler.repositories import in_memory as in_memory_repo
...
def test_tenant_isolation_tasks(fresh_app: TestClient) -> None:
...
def test_tenant_isolation_dag(fresh_app: TestClient) -> None:
    token_acme = _token(tenant_id="tenant-acme")
    token_globex = _token(tenant_id="tenant-globex")

    r1 = fresh_app.get(
        "/api/v1/scheduler/dag",
        headers={"Authorization": f"Bearer {token_acme}"},
    )
    r2 = fresh_app.get(
        "/api/v1/scheduler/dag",
    ...
    assert r1.status_code == 200, r1.text
    assert r2.status_code == 200, r2.text
    # Each tenant sees its own DAG (3 nodes by seed contract)
    assert len(r1.json()) >= 3
    assert len(r2.json()) >= 3
    # Same task_ids but different tenant scope — graph is per-tenant
...
def test_cross_tenant_trigger_404(fresh_app: TestClient) -> None:
    token_acme = _token(tenant_id="tenant-acme")
    create = fresh_app.post(
        "/api/v1/scheduler/tasks",
        json={
            "name": "Acme Trigger Protected",
            "cron_expression": "0 3 * * *",
        },
        headers={"Authorization": f"Bearer {token_acme}"},
    )
...
def test_health_anonymous_ok(fresh_app: TestClient) -> None:
    r = fresh_app.get("/api/v1/scheduler/health")
