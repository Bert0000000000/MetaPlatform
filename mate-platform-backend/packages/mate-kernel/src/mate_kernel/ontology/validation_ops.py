...
def validate_model(ot: ObjectType) -> dict[str, Any]:
    ...
    slugs: dict[str, int] = {}
    pk_set = {p.rid.rid if hasattr(p.rid, "rid") else str(p.rid) for p in ot.primary_key}
    ...
    for pk in pk_set:
        if pk not in {p.rid.rid if hasattr(p.rid, "rid") else str(p.rid) for p in ot.properties}:
            errors.append(f"PK 引用不存在于 properties: {pk}")

    return {"valid": not errors, "errors": errors, "warnings": warnings, "rid": rid}
...
def validate_instance(
    ...
    return {"valid": not errors, "errors": errors, "warnings": warnings, "class_rid": ot.rid.rid}
