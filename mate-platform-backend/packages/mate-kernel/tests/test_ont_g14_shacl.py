...
def _shape(**kw) -> NodeShape:
    return NodeShape(target_class=EMP, property_shapes=(PropertyShape(path=PROP, **kw),))
...
class TestClosed:
    ...
    def test_closed_violation_on_extra_property(self):
        r = _run(
            [_ind("i1", EMP, {PROP: "bob", PROP_AGE: 3})],
            [NodeShape(EMP, (PropertyShape(path=PROP),), closed=True)],
        )
        assert not r["conforms"]
...
class TestSeverity:
    def test_warning_does_not_break_conforms(self):
        ...
        shape = NodeShape(
            EMP, (PropertyShape(path=PROP, pattern=r"^EMP-\d+$", severity="Warning"),)
        )
    ...
    def test_info_result_keeps_conforms(self):
        shape = NodeShape(EMP, (PropertyShape(path=PROP, datatype="integer", severity="Info"),))
    ...
    def test_violation_still_breaks_conforms_and_counts(self):
        shapes = [
            NodeShape(EMP, (PropertyShape(path=PROP, pattern=r"^EMP-\d+$", severity="Warning"),)),
            NodeShape(EMP, (PropertyShape(path=PROP_AGE, datatype="integer", severity="Info"),)),
    ...
    def test_node_shape_closed_severity(self):
        shape = NodeShape(EMP, (PropertyShape(path=PROP),), closed=True, severity="Warning")
...
class TestNotConstraint:
    ...
    def test_not_with_severity_warning(self):
        ...
        r = _run(
            [_ind("i1", EMP, {PROP: "ok"})],
            [NodeShape(EMP, (PropertyShape(path=PROP, not_shape=neg, severity="Warning"),))],
        )
        assert r["conforms"]
        assert r["violations"][0]["constraint"] == "not"
...
class TestLanguageIn:
    ...
    def test_language_in_accepts_jsonld_carrier(self):
        ...
        r = _run(
            [_ind("i1", EMP, {PROP: {"@value": "hello", "@language": "en"}})],
...
class TestQualifiedValueShape:
    ...
    def test_qualified_min_count_pass(self):
        r = _run(
            [_ind("i1", EMP, {PROP: ["EMP-1", "EMP-2", "other"]})],
    ...
    def test_qualified_min_count_violation(self):
        r = _run(
            [_ind("i1", EMP, {PROP: ["EMP-1", "nope"]})], [self._qshape(qualified_min_count=2)]
        )
        assert not r["conforms"]
    ...
    def test_qualified_max_count_violation(self):
        r = _run(
            [_ind("i1", EMP, {PROP: ["EMP-1", "EMP-2", "EMP-3"]})],
...
class TestShaclReasoning:
    EMPSHAPE = NodeShape(EMP, (PropertyShape(path=PROP, min_count=1, datatype="string"),))

    def test_subclass_instance_targeted(self):
        """manager ⊑ employee：employee 的 shape 验证 manager 实例（缺 name 报违例）。"""
        axioms = [(MGR, EMP)]
        r = validate_shacl([_ind("m1", MGR, {})], [self.EMPSHAPE], subclass_axioms=axioms)
        assert not r["conforms"]
        assert r["violations"][0]["constraint"] == "minCount"
    ...
    def test_transitive_closure_targeting(self):
        ...
        r = validate_shacl(
            [_ind("l1", LEAD, {PROP: "bob"})], [self.EMPSHAPE], subclass_axioms=axioms
        )
    ...
    def test_sh_class_subclass_satisfaction(self):
        ...
        axioms = [(MGR, EMP)]
        shape = NodeShape(EMP, (PropertyShape(path=PROP_MGR, node_class=EMP),))
        r = validate_shacl(
            [_ind("m1", MGR, {PROP: "boss", PROP_MGR: "m2"}), _ind("m2", MGR, {PROP: "x"})],
            [shape],
    ...
    def test_multi_parent_closure(self):
        ...
        pshape = NodeShape(PERSON, (PropertyShape(path=PROP, min_count=1),))
        r = validate_shacl(
            [_ind("m1", MGR, {PROP: "boss"})], [self.EMPSHAPE, pshape], subclass_axioms=axioms
        )
    ...
    def test_unrelated_branch_not_targeted(self):
        ...
        r = validate_shacl([_ind("x1", OTHER, {})], [self.EMPSHAPE], subclass_axioms=axioms)
        assert r["conforms"] and r["stats"]["nodes_validated"] == 0
