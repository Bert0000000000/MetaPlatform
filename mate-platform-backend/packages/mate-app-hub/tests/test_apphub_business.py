...
def test_register_app_invalid_category(
    client: TestClient, auth_headers_acme: dict[str, str]
) -> None:
    """POST /apps with unknown category -> 422."""
    r = client.post(
        "/api/v1/apphub/apps",
        json={"name": "Bad Cat", "code": "bad-cat", "category": "nonexistent", "version": "1.0.0"},
        headers=auth_headers_acme,
    )
...
def test_register_app_category_no_group(
    client: TestClient, auth_headers_acme: dict[str, str]
) -> None:
    ...
    r = client.post(
        "/api/v1/apphub/apps",
        json={"name": "Dup", "code": "kb", "category": "knowledge", "version": "1.0.0"},
        headers=auth_headers_acme,
    )
...
def test_register_app_invalid_version(
    client: TestClient, auth_headers_acme: dict[str, str]
) -> None:
    """POST /apps with non-semver version -> 422."""
    r = client.post(
        "/api/v1/apphub/apps",
        json={"name": "Bad Ver", "code": "badver", "category": "knowledge", "version": "v1.0"},
        headers=auth_headers_acme,
    )
    assert r.status_code == 422, r.text
...
def test_register_app_emits_outbox(
    client: TestClient, auth_headers_acme: dict[str, str], outbox: object
) -> None:
    """POST /apps emits apphub.app.registered."""
    client.post(
        "/api/v1/apphub/apps",
        json={"name": "Event App", "code": "event-app", "category": "platform", "version": "2.0.0"},
        headers=auth_headers_acme,
    )
...
def test_update_app_same_version_rejected(
    client: TestClient, auth_headers_acme: dict[str, str]
) -> None:
    """PATCH /apps/{code} with same version -> 409."""
    client.post(
        "/api/v1/apphub/apps",
        json={"name": "Same Ver", "code": "samever", "category": "knowledge", "version": "1.0.0"},
        headers=auth_headers_acme,
    )
    r = client.patch(
...
def test_update_app_invalid_version(client: TestClient, auth_headers_acme: dict[str, str]) -> None:
    ...
    client.post(
        "/api/v1/apphub/apps",
        json={"name": "Inv Ver", "code": "invver", "category": "knowledge", "version": "1.0.0"},
        headers=auth_headers_acme,
    )
    r = client.patch(
...
def test_update_app_emits_outbox(
    client: TestClient, auth_headers_acme: dict[str, str], outbox: object
) -> None:
    """PATCH /apps/{code} emits apphub.app.updated."""
    client.post(
        "/api/v1/apphub/apps",
        json={"name": "UpdEvt", "code": "updevt", "category": "data", "version": "1.0.0"},
        headers=auth_headers_acme,
    )
    client.patch(
...
def test_delete_app_emits_outbox(
    client: TestClient, auth_headers_acme: dict[str, str], outbox: object
) -> None:
    """DELETE /apps/{code} emits apphub.app.deleted."""
    client.post(
        "/api/v1/apphub/apps",
        json={"name": "DelEvt", "code": "delevt", "category": "knowledge", "version": "1.0.0"},
        headers=auth_headers_acme,
    )
...
def test_create_group_success(client: TestClient, auth_headers_acme: dict[str, str]) -> None:
    ...
    r = client.post(
        ...
        json={"name": "Custom Group", "code": "custom-group", "icon": "star", "sort_order": 50},
        headers=auth_headers_acme,
    )
    assert r.status_code == 201, r.text
...
def test_delete_empty_group_success(client: TestClient, auth_headers_acme: dict[str, str]) -> None:
    ...
    r = client.delete(
        "/api/v1/apphub/groups/empty-grp",
        headers=auth_headers_acme,
    )
    assert r.status_code == 200, r.text
...
def test_create_module_unknown_app(client: TestClient, auth_headers_acme: dict[str, str]) -> None:
    ...
    r = client.post(
        "/api/v1/apphub/modules",
        json={"name": "Orphan", "code": "orphan-mod", "app_code": "nope"},
        headers=auth_headers_acme,
    )
    assert r.status_code == 422, r.text
...
def test_create_module_duplicate(client: TestClient, auth_headers_acme: dict[str, str]) -> None:
    ...
    r = client.post(
        "/api/v1/apphub/modules",
        json={"name": "Dup Mod", "code": "kb-search", "app_code": "kb"},
        headers=auth_headers_acme,
    )
    assert r.status_code == 409, r.text
...
def test_create_page_unknown_module(client: TestClient, auth_headers_acme: dict[str, str]) -> None:
    ...
    r = client.post(
        "/api/v1/apphub/pages",
        json={"name": "Orphan Page", "code": "orphan-page", "module_code": "nope"},
        headers=auth_headers_acme,
    )
    assert r.status_code == 422, r.text
...
def test_create_template_invalid_type(
    ...
    r = client.post(
        "/api/v1/apphub/templates",
        json={"name": "Bad Type", "code": "badtype-tpl", "template_type": "unknown"},
        headers=auth_headers_acme,
    )
    assert r.status_code == 422, r.text
...
def test_create_template_duplicate(client: TestClient, auth_headers_acme: dict[str, str]) -> None:
    ...
    r1 = client.post(
        "/api/v1/apphub/templates",
        json={"name": "Original Tpl", "code": "dup-tpl", "template_type": "workflow"},
        headers=auth_headers_acme,
    )
    ...
    r = client.post(
        "/api/v1/apphub/templates",
        json={"name": "Dup Tpl", "code": "dup-tpl", "template_type": "form"},
        headers=auth_headers_acme,
    )
    assert r.status_code == 409, r.text
...
def test_app_tenant_isolation(
    client: TestClient, auth_headers_acme: dict[str, str], auth_headers_globex: dict[str, str]
) -> None:
