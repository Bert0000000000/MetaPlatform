...
def test_real_executor_submit_form_returns_submission_id(
    ...
    result = asyncio.run(real_executor.submit_form(ctx, action, {"name": "test"}))
...
def test_real_executor_trigger_flow_returns_process_instance_id(
    ...
    action = RuntimeAction(
        action_id="act-2",
        action_type="trigger_flow",
        target="process.approval",
    )
    import asyncio

    result = asyncio.run(real_executor.trigger_flow(ctx, action, {"amount": 1000}))
    assert result.success is True
...
def test_real_executor_call_api_returns_call_id(
    ...
    result = asyncio.run(real_executor.call_api(ctx, action, {"input": "hello"}))
    assert result.success is True
