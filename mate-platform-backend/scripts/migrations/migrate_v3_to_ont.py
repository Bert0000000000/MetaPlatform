...
async def migrate(conn) -> dict:
    ...
    for ins in instances:
        ...
        props = (
            ins["props"]
            if isinstance(ins["props"], dict)
            else __import__("json").loads(ins["props"])
        )
        ...
        await conn.execute(
            ...
            rid,
            TENANT,
            f"ont.{TENANT}.obj.{name}.v1",
            __import__("json").dumps({f"ont.{TENANT}.prop.{k}.v1": v for k, v in props.items()}),
            pk_val,
        )
    ...
    for rel in relations:
        rid = (
            f"ont.{TENANT}.lnk.mig-{rel['relation_type']}.{rel['src_id'][-4:]}.{rel['dst_id'][-4:]}"
        )
        await conn.execute(
            ...
            rid,
            TENANT,
            f"ont.{TENANT}.link.mig-{rel['relation_type']}.v1",
            rel["src_id"],
            rel["dst_id"],
        )
    return {"classes": len(classes), "instances": len(instances), "relations": len(relations)}


async def verify(conn) -> dict:
    obj_n = await conn.fetchval(
        f"SELECT count(*) FROM {V2_OBJ} WHERE rid LIKE 'ont.{TENANT}.obj.%' AND rid NOT LIKE '%employee%' AND rid NOT LIKE '%order%' AND rid NOT LIKE '%customer%' AND rid NOT LIKE '%goods%' AND rid NOT LIKE '%bad%' AND rid NOT LIKE '%dup%' AND rid NOT LIKE '%good%' AND rid NOT LIKE '%thing%' AND rid NOT LIKE '%bare%' AND rid NOT LIKE '%ver-demo%'"
    )
    ind_n = await conn.fetchval(
        f"SELECT count(*) FROM {V2_IND} WHERE rid LIKE 'ont.{TENANT}.ind.mig-customer.%' OR rid LIKE 'ont.{TENANT}.ind.mig-order.%'"
    )
    lnk_n = await conn.fetchval(
        f"SELECT count(*) FROM {V2_LNK} WHERE rid LIKE 'ont.{TENANT}.lnk.mig-places.%'"
    )
    # 抽样比对 3 条
    samples = []
    ...
    return {
        "v2_objects": obj_n,
        "v2_individuals": ind_n,
        "v2_links": lnk_n,
        "samples": [
            {"key": k, "got": got, "want": want, "match": got == want} for k, got, want in samples
        ],
