...
async def _run_steps(client, tag: str, steps, token: str, approve: bool | None):
    ...
    if os.environ.get("SMOKE_RESTART_WORKER") == "1":
        subprocess.run(
            ["docker", "restart", "mate-temporal-worker"], check=False, capture_output=True
        )
    ...
    handle = await client.start_workflow(
        ...
        id=f"plan-5k-{tag}-{int(time.time() * 1000)}",
...
async def main() -> int:
    ...
    login = _post("/api/v1/iam/auth/login", {"username": "admin", "password": "admin123"})
    ...
    print(
        f"[env] gateway login OK; {len(types)} object types; target={target['rid']} props={props}",
        flush=True,
    )
    ...
    _, r = await _run_steps(
        ...
        [
            WorkflowStep(
                ...
                kind="propose",
                target=target["rid"],
                payload={"action_kind": "create_instance", "props": props},
            )
        ],
        token,
        approve=False,
    ...
    h = await client.start_workflow(
        PlanWorkflow.run,
        inp.model_dump(mode="json"),
        id=f"plan-lt-{int(time.time() * 1000)}",
        task_queue=TASK_QUEUE,
    )
    ...
    ok = all(
        s == "completed" or (k.startswith("propose") and s == "aborted") for k, g, s in results
    )
