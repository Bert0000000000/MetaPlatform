...
async def _fresh_worker() -> None:
    subprocess.run(["docker", "restart", "mate-temporal-worker"], check=False, capture_output=True)
...
async def main() -> int:
    ...
    h = await client.start_workflow(
        ...
        id=f"plan-callag-{int(time.time() * 1000)}",
        task_queue=TASK_QUEUE,
    )
    gate = await _wait_hitl(h)
    await h.signal(
        PlanWorkflow.review,
        ReviewSignal(step_id=gate.split(":", 1)[1], approved=True, reviewer="e2e"),
    )
    r5 = await h.result()
    ...
    await h2.signal(
        PlanWorkflow.review,
        ReviewSignal(step_id=gate2.split(":", 1)[1], approved=True, reviewer="long-task-demo"),
    )
    rlt = await h2.result()
    print(f"long_task: {gate2} waited={waited:.0f}s mid={mid} -> {rlt['status']}", flush=True)
