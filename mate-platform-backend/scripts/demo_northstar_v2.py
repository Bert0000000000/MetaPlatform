...
def _req(path: str, method: str = "GET", body: dict | None = None, token: str = ""):
    req = urllib.request.Request(
        GW + path, data=json.dumps(body).encode() if body is not None else None,
...
async def main() -> int:
    token = _req("/api/v1/iam/auth/login", "POST",
                 {"username": "admin", "password": "admin123"})["accessToken"]
    stage(1, f"登录 OK（token {len(token)} chars）")

    orders = _req("/api/v1/ont/v2/object-query", "POST", {
        "source": "ont.tenant-default.obj.crm.order.v1", "filters": [],
        "paging_limit": 5,
    }, token)
    rows = orders.get("rows") or []
    if not rows:
        print("无订单实例可演示（先 seed）"); return 1
    order = rows[0]
    order_rid = order["__rid__"]
    stage(2, f"真实订单数据: {order_rid} "
    ...
    steps = [
        WorkflowStep(step_id="evidence", kind="evaluate_object_set",
                     target="ont.tenant-default.obj.crm.order.v1",
                     payload={"filter": {}, "paging": {"limit": 3}}),
        WorkflowStep(step_id="propose", kind="propose", target=act,
                     payload={"parameters": {"decision": "confirm"},
                              "target_iid": order_rid,
                              "impact_summary": f"北极星 v2: 复核订单 {order_rid}"}),
    ]
    inp = workflow_input_from_steps(tenant_id="tenant-default",
                                    steps=steps, token=token)
    handle = await client.start_workflow(
        PlanWorkflow.run, inp.model_dump(mode="json"),
        id=f"northstar-v2-{int(time.time()*1000)}", task_queue=TASK_QUEUE)
    ...
    await handle.signal(PlanWorkflow.review, ReviewSignal(
        step_id=gate.split(":", 1)[1], approved=True, reviewer="northstar-v2"))
    ...
    for r in result.get("results", []):
        ...
        if isinstance(out, dict):
            proposal_id = (out.get("proposal_id")
                           or (out.get("executed") or {}).get("proposal_id", ""))
            if proposal_id:
                break
    if not proposal_id:
        print("未提取到 proposal_id，跳过 revert"); return 1
    from mate_tech_orchestrator.temporal_workflow import RevertWorkflow
    rh = await client.start_workflow(
        RevertWorkflow.run,
        {"tenant_id": "tenant-default", "proposal_id": proposal_id,
         "token": token},
        id=f"northstar-revert-{int(time.time()*1000)}", task_queue=TASK_QUEUE)
    ...
    with open("northstar-v2-result.json", "w", encoding="utf-8") as f:
        json.dump({"stages": STAGES, "final": str(result["status"]),
