...
for i, r in enumerate(app.routes):
    ...
    if cls == "_IncludedRouter":
        ...
        print(
            f"  [{i}] _IncludedRouter prefix={getattr(r, 'prefix', '')!r} sub_cls={type(sub).__name__} sub_routes_count={len(getattr(sub, 'routes', []))}"
        )
        if sub is not None:
            for j, sr in enumerate(sub.routes[:3]):
                print(
                    f"     sub[{j}] {type(sr).__name__} path={getattr(sr, 'path', None)} methods={getattr(sr, 'methods', None)}"
                )
    else:
