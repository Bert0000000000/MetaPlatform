...
def _call(method: str, path: str, token: str, payload: dict | None = None,
          tenant: str = TENANT, timeout: float = 30.0):
...
def main() -> int:
    ...
    status, body = _call("POST", "/api/v1/ont/v2/shacl/validate", token,
                         {"target_class": emp})
    a1 = status == 200 and body.get("conforms") is True
    check("shacl integrated conforms (type-synth shape + stored instances)",
          a1, f"HTTP {status} conforms={body.get('conforms') if isinstance(body, dict) else '?'}")

    # A2 stateless 负例：pattern 不匹配 + closed 多余属性
    status, body = _call("POST", "/api/v1/ont/v2/shacl/validate", token, {
        "target_class": emp,
        "individuals": [
            {"rid": "ont.tenant-default.ind.g21.bad.v1", "class_rid": emp,
             "props": {emp_name: "Bad-Name",
                       f"ont.{TENANT}.prop.g21-extra.v1": 1}},
        ],
        "property_shapes": [
    ...
    constraints = {v["constraint"] for v in (body.get("violations") or [])} \
        if isinstance(body, dict) else set()
    a2 = (status == 200 and body.get("conforms") is False
          and {"pattern", "closed"} <= constraints)
    check("shacl stateless negative (pattern+closed)",
          a2, f"HTTP {status} constraints={sorted(constraints)}")

    print("== B. 治理面（DATA-D6/D7）==", flush=True)

    # B1 lineage 登记边 + 子图
    status, edge = _call("POST", "/api/v1/data/lineage/edges", token, {
    ...
    status, graph = _call("GET", "/api/v1/data/lineage/graph"
                          "?entity=ont.x.obj.orders.v1", token)
    nodes = {n.get("id") for n in (graph or {}).get("nodes", [])}
    b1 = b1_create and status == 200 and nodes == {
        "ont.x.obj.orders.v1", "ont.x.obj.orders-ads.v1"}
    check("lineage edge + subgraph", b1, f"HTTP {status} nodes={sorted(nodes)}")

    # B2 quality：required 正例 + type 反例 → run → results
    status, _ = _call("POST", "/api/v1/data/quality/rules", token, {
        "entity_id": "src-mysql-orders", "field": "amount",
        "rule_type": "required", "params": {"table": "orders"}})
    b2_rule1 = status == 200
    status, _ = _call("POST", "/api/v1/data/quality/rules", token, {
        "entity_id": "src-mysql-orders", "field": "amount",
        "rule_type": "type",
    ...
    b2_run = (status == 200 and run.get("rules_executed", 0) >= 2
              and run.get("passed", 0) >= 1 and run.get("failed", 0) >= 1)
    status, stored = _call("GET", "/api/v1/data/quality/results", token)
    b2_stored = status == 200 and stored.get("total", 0) >= 2
    check("quality rules run (required pass + type fail)",
          ...
          if isinstance(run, dict) else f"HTTP {status}")
    ...
    check("catalog search hits source+product",
