...
def _call(
    method: str, path: str, token: str, payload: dict | None = None, tenant: str = TENANT
) -> tuple[int, object]:
...
def main() -> int:
    ...
    for rid, name in ((rid_p, "person"), (rid_e, "employee"), (rid_m, "manager")):
        status, body = _call("POST", "/api/v1/ont/v2/object-types", token, type_body(rid, name))
    ...
    for name, cls in (("alice", rid_p), ("bob", rid_e), ("carol", rid_m)):
        ind = {
            "rid": f"ont.{TENANT}.ind.g21.{name}.v1",
            "class_rid": cls,
    ...
    ax1 = {
        "rid": f"ont.{TENANT}.ax.g21-employee-person.v1",
        "kind": "subclass",
        "operands": [rid_e, rid_p],
        "enabled": True,
    }
    ax2 = {
        "rid": f"ont.{TENANT}.ax.g21-manager-employee.v1",
        "kind": "subclass",
        "operands": ["g21-manager", "g21-employee"],
        "enabled": True,
    }
    for ax in (ax1, ax2):
        status, body = _call("POST", "/api/v1/ont/v2/reasoning/axioms", token, ax)
        check(f"register axiom {ax['rid'].rsplit('.', 2)[-2]}", status == 200, f"HTTP {status}")
    ...
    r1_ok = status == 200 and out["classification"]["emp-1"]["inferred"] == ["agent", "person"]
    check("reasoning/run R1 closure", r1_ok, f"HTTP {status}")

    print("== 5. ObjectSet 祖先查询命中后代实例（闭包扩展）==", flush=True)
    for cls, expect, label in (
        (rid_p, {"alice", "bob", "carol"}, "person"),
        ...
        status, body = _call(
            ...
            token,
            {"class_rid": cls, "filter_expr": "g21-name", "paging_limit": 100},
        )
        names = set()
        if isinstance(body, dict):
            for r in body.get("results") or []:
                ...
                if v:
                    names.add(str(v))
        check(
            f"object-set query {label}",
            status == 200 and names == expect,
            f"HTTP {status} got {sorted(names)} expect {sorted(expect)}",
        )

    print("== 6. Function 注册面 ==", flush=True)
    fn = {
        "rid": f"ont.{TENANT}.fn.g21-classify.v1",
        "language": "python",
        "version": 1,
    ...
    fn_listed = (
        status2 == 200
        and isinstance(body2, list)
        and any(f2.get("rid") == fn["rid"] for f2 in body2)
    )
    check("function register+list", fn_created and fn_listed, f"HTTP {status}/{status2}")
    ...
    status, _ = _call(
        ...
        "/api/v1/ont/v2/object-sets/query",
        token,
        {"class_rid": "ont.tenant-acme.obj.x.v1", "filter_expr": "y"},
    )
