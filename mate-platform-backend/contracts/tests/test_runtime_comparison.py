...
def module():
    path = ROOT / "scripts/compare_runtime.py"
    spec = importlib.util.spec_from_file_location("runtime_compare", path)
...
def test_comparison_reports_both_directions() -> None:
    mod = module()
    contracts = {("get", "/api/v1/example/items"): "implemented"}
    runtime = {("post", "/api/v1/example/items")}
...
def test_planned_and_placeholder_may_be_absent() -> None:
    mod = module()
    contracts = {("get", "/api/v1/future"): "planned", ("post", "/api/v1/mock"): "placeholder"}
