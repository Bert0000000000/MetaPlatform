...
def normalize(domain: str, path: str) -> tuple[str, str]:
    ...
    if domain == "kb":
        return "kb", path.replace("/api/v1/app-kb", "/api/v1/kb")
    if domain == "llmgw":
        return "llmgw", path.replace("/api/v1/llm", "/api/v1/llmgw")
    return domain, path


def test_existing_operations_are_conserved() -> None:
    targets = {
        d: yaml.safe_load((ROOT / f"openapi/services/{d}.yaml").read_text(encoding="utf-8"))
        for d in ["iam", "dashboard", "msg", "obs", "mcp", "llmgw", "ont", "rag", "agent", "kb"]
    }
...
def test_every_migrated_operation_has_governance() -> None:
    for path in (ROOT / "openapi/services").glob("*.yaml"):
        ...
        for _route, item in doc.get("paths", {}).items():
            if _route in {"/healthz", "/readyz", "/metrics"}:
                continue
            for method, op in item.items():
                ...
                assert op.get("x-mate-implementation-status") in {
                    "implemented",
                    "placeholder",
                    "planned",
                }
