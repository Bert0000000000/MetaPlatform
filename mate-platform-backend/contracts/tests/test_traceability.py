...
def load_validator():
    path = ROOT / "scripts/validate_traceability.py"
    spec = importlib.util.spec_from_file_location("trace", path)
...
def test_status_combinations_are_truthful() -> None:
    matrix = yaml.safe_load(
        (WORKSPACE / "docs/active/delivery/REQUIREMENT-MATRIX.yaml").read_text(encoding="utf-8")
    )
