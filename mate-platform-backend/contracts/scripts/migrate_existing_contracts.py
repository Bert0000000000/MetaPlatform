...
def camel(domain: str, method: str, path: str) -> str:
    words = [domain, method] + [
        p.strip("{}:").replace("-", "_") for p in path.split("/") if p and p not in {"api", "v1"}
    ]
...
def target(domain: str, path: str) -> tuple[str, str]:
    if domain == "iam" and path.startswith("/api/v1/dashboard"):
        return "dashboard", path
    if domain == "kb":
        return domain, path.replace("/api/v1/app-kb", "/api/v1/kb")
    if domain == "llmgw":
        return domain, path.replace("/api/v1/llm", "/api/v1/llmgw")
    return domain, path
...
def status(domain: str, path: str) -> str:
    if domain in PLACEHOLDER:
        return "placeholder"
    if domain == "llmgw" and (path.endswith("/stream") or path.endswith("/embeddings")):
        return "placeholder"
    if domain == "ont" and ("/instances" in path or path.endswith("/sparql")):
        return "placeholder"
    if domain == "msg" and path.endswith("/topics"):
        return "placeholder"
    if domain == "iam" and ("/auth/" in path or path.endswith("/sso-providers")):
        return "placeholder"
    return "implemented"
...
def blank(domain: str) -> dict:
    return {
        "openapi": "3.1.0",
        "info": {
            "title": f"Mate Platform {domain} API",
            "version": "1.0.0",
            "x-mate-owner": OWNERS[domain],
        },
        "servers": [{"url": "/"}],
        "tags": [],
        "security": [{"bearerAuth": []}],
...
def migrate() -> None:
    ...
    for source_domain, source in SOURCES.items():
        ...
        component_targets = (
            [source_domain, "dashboard"] if source_domain == "iam" else [source_domain]
        )
        for component_domain in component_targets:
            for name, value in (src.get("components") or {}).items():
                if name == "securitySchemes":
                    continue
                docs[component_domain].setdefault("components", {}).setdefault(name, {}).update(
                    deepcopy(value or {})
                )
        for path, item in (src.get("paths") or {}).items():
            ...
            for method, op in new_item.items():
                ...
                op.pop("parameters", None) if any(
                    isinstance(x, dict)
                    and str(x.get("name", "")).lower() in {"x-tenant-id", "x-mate-tenant-id"}
