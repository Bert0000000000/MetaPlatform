...
OWNERS = {
    ...
    "dw": "digital-workforce",
    "apphub": "application-platform",
    "arch": "enterprise-architecture",
    "wfe": "workflow-platform",
    "a2a": "ai-protocols",
}
MAP = {
    "data": "data",
...
def generate() -> None:
    ...
    docs = {
        d: {
            "openapi": "3.1.0",
            "info": {"title": f"Mate Platform {d} API", "version": "1.0.0", "x-mate-owner": o},
            "servers": [{"url": "/"}],
            "tags": [{"name": d, "description": f"{d} domain operations"}],
            "security": [{"bearerAuth": []}],
            "paths": {},
            "components": deepcopy(src.get("components") or {}),
        }
        for d, o in OWNERS.items()
    }
    for old_path, item in (src.get("paths") or {}).items():
        ps = parts(old_path)
        if not ps or ps[0] not in MAP:
            continue
        source_prefix = ps[0]
        domain = MAP[source_prefix]
        ...
        for method, op in new_item.items():
            ...
            for parameter in op.get("parameters", []):
                if (
                    ...
                    and parameter.get("$ref") == "#/components/parameters/TenantId"
                ):
                    continue
                if (
                    isinstance(parameter, dict)
                    and parameter.get("$ref") == "#/components/parameters/Id"
                ):
                    normalized.append(
    ...
    (OUT.parent / "migration_exclusions.yaml").write_text(
        yaml.safe_dump(exclusions, sort_keys=False, allow_unicode=True), encoding="utf-8"
    )
