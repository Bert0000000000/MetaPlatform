    ...
    for route, item in (doc.get("paths") or {}).items():
        ...
        for method, operation in (item or {}).items():
            ...
            if not opid:
                ...
                errors.append(f"{label}: duplicate operationId {opid}")
            else:
                seen.add(opid)
            for key in (
                "summary",
            ...
            for parameter in operation.get("parameters", []):
                if (
                    ...
                    in {"x-tenant-id", "x-mate-tenant-id"}
                ):
...
def validate_all(root: Path) -> list[str]:
    ...
    for path in sorted((root / "services").glob("*.yaml")):
        errors.extend(
            validate_document(path, yaml.safe_load(path.read_text(encoding="utf-8")) or {})
        )
    return errors
