...
def sanitize_document(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    for raw_path, item in doc.get("paths", {}).items():
        ...
        for method, op in (item or {}).items():
            ...
            if isinstance(body, dict) and not body.get("content"):
    ...
    info.setdefault(
        "description", f"Canonical contract for {info.get('title', 'Mate Platform service')}."
    )
    info.setdefault(
        "contact",
        {"name": "Mate Platform API Governance", "email": "api-governance@metaplatform.local"},
    )
    return doc
