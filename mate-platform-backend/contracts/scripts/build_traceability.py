...
def prd_for(domain: str) -> str:
    if domain in PRD_DIR:
        ...
        if files:
            return files[-1].relative_to(WORKSPACE).as_posix()
            ...
            for method, op in path_item.items():
                ...
                for req in op["x-mate-requirements"]:
                    ...
                    if status == "implemented" and item.get("runtimeModule"):
                        ...
                        requirement["tests"] = [
                            "mate-platform-backend/contracts/tests/test_runtime_comparison.py"
                        ]
    out = {"version": 1, "requirements": dict(sorted(requirements.items()))}
    target = WORKSPACE / "docs/active/delivery/REQUIREMENT-MATRIX.yaml"
