...
def compare_operations(
    contracts: dict[tuple[str, str], str], runtime: set[tuple[str, str]]
) -> dict[str, list[str]]:
    c = {(m.lower(), normalize_path(p)): s for (m, p), s in contracts.items()}
    r = {(m.lower(), normalize_path(p)) for m, p in runtime if p not in META}
    required = {op for op, status in c.items() if status == "implemented"}
    return {
        "missingInRuntime": [f"{m.upper()} {p}" for m, p in sorted(required - r)],
        "undocumentedRuntimeOperation": [f"{m.upper()} {p}" for m, p in sorted(r - set(c))],
    }
        ...
        for route, item in doc.get("paths", {}).items():
            ...
            for method, op in item.items():
                if method in METHODS:
                    result[(method, route)] = op["x-mate-implementation-status"]
    return result


def load_runtime() -> set[tuple[str, str]]:
    result = set()
    index_path = CONTRACTS / "runtime/index.json"
    if not index_path.exists():
        raise FileNotFoundError("run runtime_openapi.py first")
