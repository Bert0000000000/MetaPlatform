...
| 1.0~1.9 | 见上「已交付」 | ✅ |
| **2.0** | **收尾 + 会话页整合** | ✅ |
| **2.1** | **生产硬化**（五条性质：副作用最多一次 / 权限撤销不扩权 / 审计可取证 / 外部进程不泄漏 / 门禁真拦） | 🟡 **已规划**：`2026-09-17-agent-product-layer-2.1-roadmap.md` |

> **2.1 范围变更说明（2026-09-17）**：此前本表把 2.1 记为「上下文感知（ADR-0065）」。
> 依据 2.0 后的外部评审（21 条差距逐条核到代码）+ 2.0 验收 §3 边界表对账，
> 2.1 改定为**生产硬化**，内部分 **2.1-A / 2.1-B / 2.1-C** 三个分组；
> 上下文感知（ADR-0065）降为 **2.1-C** 的一条，**ADR 仍是 Proposed，不得偷偷升格**。
>
> **2.1 挂靠在平台级季度计划之下**：`MetaPlatform-调整优化方案执行计划-2026-09-17.md`
> （13 周 / 6 Sprint，同一基线 `main@24976a4`）。**对外批次号一律用它那套 `MP-*`**，
> `2.1-A/B/C` 只是阅读分组；两份的分工与该计划**未覆盖的 9 个批次**见
> `2026-09-17-agent-product-layer-2.1-roadmap.md` §8。
