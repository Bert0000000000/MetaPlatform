...
| 2026-08-17 | **北极星 live 实机验证通过（读+想+写全链 HTTP 级）** | 真 uvicorn + 真 PG（KERNEL_BACKEND=pg, ONT_EMBEDDER=hash, 8307）：发布 2 类型（含 domain:finance 打标）+3 实例+1 ActionType → ①OAG 中文检索命中卡片(rid+matched 值) → ②agent-tools marking 闸两面（无标记缺 query_nsd_ledger/带标记出现；16 租户类型即发布即工具）→ ③IR 查询 amount>100000∧open 只中 o1 + result_schema + 分组聚合 → ④propose(pending+expected_diff) → ⑤**negative 409「proposal is pending; apply requires confirmed」+ o2 数据零变化** → ⑥confirm(confirmed_by=rouge) → ⑦apply 200 → o1 review-reason 写回 + proposal=applied。实机暴露并修复 5 项：hash 逃生阀在无 key 时失效 / value_text 误存 prop rid / to_thread×threading.local 租户兜底失效（改显式传参第二道防线）/ ProposalNotConfirmed 未映射 409 / HashEmbedder 中文整句单 token（补 CJK bigram）。demo 数据已清理；ont 179 passed + kernel 476 passed 全绿。 |
| 2026-08-17 | **MP-SAL-04b → Accepted（文本→本体 Ingest）** | 用户场景驱动补腿：proposal kind 泛化（action/create_instance/model_type）+ mark_applied 四态守卫 + execute 执行腿（建实例 rid 生成/建类型人审落库，action 类拒绝旁路）+ ont_proposal.kind 持久化 + REST propose-instance/propose-type/execute 三端点 + copilot 提议工具 ×2（confirm/execute 永非 LLM 工具）。Live 验证用户原场景全通：文本→建类型(409 negative→confirm→execute)→建实例(6 字段全对落库)→query_supplier 即时上工具面。kernel 487 passed。证据 `MP-SAL-04B-ACCEPTANCE.md`。 |
| 2026-08-17 | **MP-SAL 运行时接线 → Accepted（SuperAI 本体能力上线）** | 用户实测发现「问系统里有多少本体，SuperAI 无本体工具」——SAL 工具建好但未接运行时。三处收口：①copilot `OntologyHttpRepo`（tech-ont v2 HTTP 适配，Bearer+租户透传）+ `run_agent_loop` 支持 ontology 工具执行（to_thread + 结果回填历史，LLM 拿真实数据作答）+ OAG 卡片与「查=直连/干=派员工」语义指引进 system prompt，app 调用点 best-effort 接线（本体面缺席降级纯调度）；②orchestrator bootstrap 种 ONTOLOGY 数字员工（3 能力绑 MCP ont_* 代理工具——AgentRole.ONTOLOGY 枚举一直在但从未注册，SuperAI 提示里因此看不到也派不出，A2A 调度由此有用）；③list_object_types 补显式租户兜底（to_thread×thread-local 同根因，租户清单曾混入他租户类型致查询 403）。Live 端到端：工具面 7 件（租户隔离）→「多少本体」直答 1（acme-team）→ query_acme_team 返回「平台组」+schema。kernel 487 / ont 177 / orchestrator 47 / copilot 相关 35 全绿。 |
| 2026-08-17 | **MP-SAL-05 P1 → Accepted（action 流程编排×本体联动 + 可视化）** | PlanRunner 四执行器落地（PROPOSE/APPLY_ACTION≡同管线/EVALUATE_OBJECTSET IR/RUN_FUNCTION 沙箱）+ **HITL 合一**（review approve = proposal confirm + execute/apply，G4 断点④收口）+ {{steps.x.y}} 数据流 + OntologyActionClient（逐请求 tenant+token 置换）+ 流程实例本体化（process-instance 对象自动上工具面）+ **GET /plans/{id}/graph 可视化图模型**（节点状态/proposal/diff/数据流边，前端 SemiGraphCanvas 可直接渲染）。Live 双服务全链验证：查超10万订单→提议→批准→w1 真实写回+PI completed+graph 全绿。orchestrator 55 + kernel 487 全绿。证据 `MP-SAL-05-ACCEPTANCE.md`。通道②（outbox→流程自动启动）留 P2。 |
