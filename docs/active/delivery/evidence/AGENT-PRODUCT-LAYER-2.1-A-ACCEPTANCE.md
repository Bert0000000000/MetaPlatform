...
| **B6** | `cowork md-lint (pymarkdownlnt)` 红（**非 required**）—— 只剩**一个**文件 | 它只查 `docs/active/specs` 与 `docs/active/delivery/evidence` 下**变更的** .md（关掉 md013/md041）。本批自己那两份的违规已修（用同一工具同参数本地验到 exit 0）。**剩下的全是 `2026-07-27-mate-platform-architecture-implementation.md` 的既有格式债**：`pymarkdownlnt` 实测，**本分支改前的 `main` 版本与现在都是 19 条，条数一字不差** —— 那篇 7 月规格文档本来就围栏缺语言、块前后缺空行，它被 lint 只是因为工作流按"本次变更的文件"挑，而 A-7 改了它 3 行 | 两件事要分开定：① **门禁的取样范围**——"改了 3 行就得替整篇旧债买单"是取样问题（只 lint **新增**文件，或给旧文档一份显式豁免清单），归维护那份工作流的人；② 那 19 条本身是格式债，但自动修（`pymarkdownlnt fix`）实测产生 **1774 行 diff 且仍剩 9 条**——那不是修，是重写一篇核心架构文档，**本批不做** |
