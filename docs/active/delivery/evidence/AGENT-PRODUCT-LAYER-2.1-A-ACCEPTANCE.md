...
| **B6** | `cowork md-lint (pymarkdownlnt)` —— **已修**（本批自己的两份 + 那篇架构文档的既有 19 条） | 工作流只查 `docs/active/specs` 与 `docs/active/delivery/evidence` 下**变更的** .md（关掉 md013/md041）。本批自己那两份的违规已修；`2026-07-27-mate-platform-architecture-implementation.md` 的 **19 条既有债**（`main` 版本与改前版本条数一字不差）也**点对点修掉**了：13 个位置，19 插 5 删——围栏前补空行（MD031）、5 个树/流程图围栏补 `text`（MD040）、列表前补空行（MD032）。**不是我先前判的"不该修"**：那个判断只对 `pymarkdownlnt fix` 成立（它产生 1774 行 diff 且仍剩 9 条），对问题本身不成立 | 本地用 CI 同款工具同参数验到 exit 0。**仍建议上游看一眼取样范围**：现在"改了 3 行就得替整篇旧文档的格式债买单"，只 lint **新增**文件会更贴合这份门禁的用途 |
| **B7** | `Architecture kernel governance` 红（**非 required，且不阻塞合并**） | **`main` 上就是红的**（`24976a4c` 的同一 job 同为 failure）。工作流自己写着「债务登记（2026-09-10）：kernel governance 检查预存失败（main 合并前即红）。**不阻塞合并**」并带 `continue-on-error: true`。本轮实测失败步骤是 `Pyright strict (kernel + tests)`，**与本批改动无关**；它让同 job 后两步 `Architecture tests` / `Import-linter contracts` 被 skipped —— 但那两项**在 `python-ci.yml` 里另有独立 job 并已通过**（且是 required），覆盖没丢 | **规模已量**（本地同命令 `pyright packages/mate-kernel packages/mate-platform packages/mate-clients tests/architecture scripts/…`）：**295 errors / 46 warnings**，分布是 `reportMissingParameterType` 162、`reportPrivateUsage` 54、`reportUnnecessaryTypeIgnoreComment` 46、`reportOperatorIssue` 27、`reportUnusedVariable` 26 —— **六类、三个包**，是一次独立工程，不是本批顺手能清的。**本批不改**：撤 `continue-on-error` 或缩 pyright 范围都是"把红点藏起来"，不是修 |
| **B8** | `github-advanced-security` 红（**非 required，且无发现**） | 当前 head 实取：`conclusion=failure`、`annotations_count=null`、`output.title=null` —— **零安全发现**。它是 GitHub 托管的 agentic 扫描通道起不来（错误发生在 GitHub 自己的模型侧），**从仓库里改不了** | 无需动作。区分方法：看 `output` 是否为空 + 业务 workflow 是否全绿 |
| **B8** | `github-advanced-security` 红（**非 required，且无发现**） | 当前 head 实取：`conclusion=failure`、`annotations_count=null`、`output.title=null` —— **零安全发现**。它是 GitHub 托管的 agentic 扫描通道起不来（错误发生在 GitHub 自己的模型侧），**从仓库里改不了** | 无需动作。区分方法：看 `output` 是否为空 + 业务 workflow 是否全绿 |
