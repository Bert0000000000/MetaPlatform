...
| **B7** | `Architecture kernel governance` 红（**非 required，且不阻塞合并**） | **`main` 上就是红的**（`24976a4c` 的同一 job 同为 failure）。工作流自己写着「债务登记（2026-09-10）：kernel governance 检查预存失败（main 合并前即红）。**不阻塞合并**」并带 `continue-on-error: true`。本轮实测失败步骤是 `Pyright strict (kernel + tests)`（`mate-*` 测试里的既有类型债），**与本批改动无关**；它让同 job 后两步 `Architecture tests` / `Import-linter contracts` 被 skipped —— 但那两项**在 `python-ci.yml` 里另有独立 job 并已通过**，覆盖没丢 | 要对齐 governance 规则与当前 import 结构（一次性工程）。**本批不改**：把 `continue-on-error` 拿掉或缩 pyright 范围都是"把红点藏起来"，不是修 |
