...
| "拆 `ga-hooks-and-tests` 为 5 个独立 job"                                                          | workflow 已有 9 个独立 `ga-*` job，但 5 个 forbid\_\* 脚本仍在 `ga-hooks-and-tests` 复合里        | 拆 5 个独立 job，命名 `ga-003-tenant` / `ga-004-acl` / `ga-005-fallback` / `ga-007-skip-tests` / `ga-010-evidence` |
...
`ga-hooks-and-tests` 保留作为聚合 job（仍跑 pre-commit + pytest），但 5 个 forbid\_\* 改为只跑对应单规则 script。
