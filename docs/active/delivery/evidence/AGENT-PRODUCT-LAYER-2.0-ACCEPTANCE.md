...
| `7965ebc5` | 订正：provider 是配好的，回显真因是 llmgw 30s 上游超时 |
| `50bfd7f8` | 调度可视化挪进右上角紧凑面板，不再占对话区 |
| `7c75d8d0` | 把 stub-fallback 回显产出显式标出来 |
| `1922957b` | 开「llmgw 回显加固」批次（定义 + 启动提示词） |

后四条是对前两条的**跟进**（用户 2026-09-17 追加「调度内容占页面太大」+ 确认
provider 已配），不是新批次。
...
2. 调度可视化放在**页面右上角、独立于消息流**的一个区域（任务图含**波次** /
   员工状态 / 证据 / 交付物 / 终态），不再散在每条消息里。
   **位置经一次调整**（`50bfd7f8`）：初版放在对话历史之上、高 46vh（实测 609px），
   把对话区挤掉一大半；现改为会话历史栏里「新建会话」**之上**的紧凑面板
   （235×203px），竖排 + 限高自滚，点「详情」用 `SheetDetail` 打开完整视图。
...
| 独立于消息流的常驻调度可视化（任务图 / 员工状态 / 波次 / 终态） | ✅ 右上角独立区域 | §5.3 |
...
| 步骤 | 观测 |
| --- | --- |
| 打开 `/superai/chat` | 右上角出现独立区域「调度任务」，位于「新建会话」**之上**（未起 run 时给一句引导文案） |
...
**几何证据**（`getBoundingClientRect`，取自刷新后的页面，调整位置之后）：

| 元素 | top | bottom | left | right |
| --- | --- | --- | --- | --- |
| 调度面板 | 164 | 368 | 1603 | 1838 |
| 「新建会话」按钮 | 388 | — | — | — |
| 会话历史栏 | — | — | ~1590 | 1850 |

面板（235×203）整体落在会话历史栏内、且**底边 368 ≤ 按钮顶边 388** —— 「在
「新建会话」之上」成立；栏在页面最右 → 也就是**页面右上角**。面板 `overflow: auto`
且限高 42vh，长任务图自己滚，**对话区拿回整页高度**（调整前那块占 609px）。

**「详情」实测**：点开是 456px 的 `SheetDetail`，里面是**完整**视图 ——
含任务图六列表格（子任务 / 波次 / 承接员工 / 依赖 / 终态 / 指令）、员工状态表、
证据（`EvidenceRenderer`）、交付物、汇总。与工作台是**同一个组件**，数据同源。

**回显标注实测**：该轮 3 名员工里 1 名是回显 —— 面板显示 `回显 1/3`（警告色）、
详情里该员工产出上方出现警示条（`data-testid="agent-team-fallback-t2"`）、
员工状态表出现「回显」标签。
...
| 调度面板**跟着「会话历史」栏一起显示/隐藏** | 用户要求的位置就是「新建会话上面」，它在侧栏内；收起侧栏 = 连同面板一起收起 | 若要面板独立于会话列表常驻，得把它从 `Sidebar` 里搬出来、改成右列的一个兄弟块——需要动布局（本次试过，风险高于收益，没做） |
...
3. **员工产出会间歇性变成 stub-fallback 回显，而 run 仍报成功**（本轮新发现，见 §5.3）。
   ...
   **30s 上游超时**撞上 reasoning 模型（`glm-5.3-flash`）。
   - **前端侧**：✅ **本批已做**——面板加「回显 n/m」（警告色）、详情里该员工的产出上方
     加警示条、员工表状态列加「回显」标签（`agentTeamRunView.isStubFallback`）。
   - **llmgw 侧**：⏭ **另立批次** `2026-09-17-llmgw-fallback-hardening.md`
     ——上游超时可配 + 员工路径禁回显。该批次定义里写清了**这不是生产安全洞**
     （硬规则 #5 已用 `is_production_profile()` 挡住），治的是 **dev 保真度**。
