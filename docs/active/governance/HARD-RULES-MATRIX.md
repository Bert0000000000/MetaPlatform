...
| ③ | **没有 tenant 上下文，不访问 repository** | `ga-hooks-and-tests`（`forbid_raw_sql`） | `.github/workflows/ga-acceptance.yml` | SEC-TENANT | 🟡 → 硬化中 | GOVERN-06 |
...
| ⑨ | **没有审计、指标、trace** | `ga-009-observability` | `.github/workflows/ga-acceptance.yml` | PLATFORM-OBS | 🟡 → compose≠Helm | GOVERN-09 |
...
> **统计**：13 条中 **7 条 ✅**、**5 条 🟡**（其中 4 条由 GOVERN-06/-09/-10 收口）、**0 条 ⏳**、**0 条 🔧**。`ga-001~013` 中已有 9 个 job 名出现，缺 `ga-003-tenant` / `ga-004-acl` / `ga-005-fallback` / `ga-007-skip-tests` / `ga-010-evidence`——这 5 个通过 `ga-hooks-and-tests` 复合 job（内嵌 `forbid_*` 脚本）走；GOVERN-10 将把这 5 个脚本名拆为独立 job，便于硬规则 × workflow 矩阵可机检。
...
- `ga-hooks-and-tests`（硬规则 ③/④/⑤/⑦/⑩ 复合 job，内嵌 `forbid_*` 脚本）
...
| # | 缺口 | 拟新增 | 关联 |
|---|---|---|---|
| G1 | `ga-hooks-and-tests` 复合 job 难追溯到单条硬规则 | 拆为 `ga-003-tenant` / `ga-004-acl` / `ga-005-fallback` / `ga-007-skip-tests` / `ga-010-evidence` 5 个独立 job | GOVERN-10 |
| G2 | 13 × 8 matrix 无机器断言 | `tests/governance/test_hard_rules_ci.py`：枚举 13 硬规则 × 期望触发 job；CI 在 `ci.yml` 新增 `gov-hard-rules-matrix` job | GOVERN-10 |
| G3 | 5 条 🟡 硬规则的真实缺口（③/⑦/⑨/⑩/⑬） | 由 GOVERN-06 / -09 / -10 闭环 | GOVERN-06 / -09 / -10 |
| G4 | 矩阵本身无 owner 验收 | 每月 1 号由 ARCH-CORE 复核，纳入 PROGRAM-BOARD 治理 row | GOVERN-01 |

## 3. 验收标准

- `pytest tests/governance/test_hard_rules_ci.py -v` 全绿；
- `grep -RE 'ga-00[1-9]|ga-01[0-3]' .github/workflows/ga-acceptance.yml` 至少 9 个 job 名出现（当前 9 个）；
- HARD-RULES-MATRIX.md 表格 104 格（13 硬规则 × 8 workflow）中 ≥90% 为非 `N/A` 命中（当前 90% 命中，剩 10% 由 GOVERN-10 拆 job 后补齐）。

---

**关联**：GOVERN-01（创建）/ GOVERN-10（机检 + 拆 job）/ ADR-0015（GA 收口）。
