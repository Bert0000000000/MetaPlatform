...
> **版本**: v1.2 | **日期**: 2026-07-27 | **模块**: APP-KB | **状态**: 正式版
>
> **vv1.1 → vv1.2 主要变更**：
> 1. **范围对齐前端实现**：根据"以独立 APP 为主"策略，PRD 描述范围与前端代码 1:1 对齐
> 2. **API 接口按 Q2=B 决策归属**：所有 /v1/kb + /v1/rag 端点归到 TECH-RAG
...
1. **范围对齐前端实现**：根据"以独立 APP 为主"策略，PRD 描述范围与前端代码 1:1 对齐
2. **API 接口按 Q2=B 归属**：本模块所有 API 端点统一归到 **TECH-RAG**
...
| 1 | 知识库版本快照/回滚 | §1.5 | ❌ | v1.4 (2 周) |
| 2 | 切片策略模板编辑 | §1.3 | 🟡 KnowledgeConfigPage 有 | v1.4 (1 周) |
| 3 | 切片审核 UI | §1.6 | ❌ 文档预览无审核 | v1.4 (1 周) |

#### X.2.2 P1 - 增强功能

| 1 | 知识库导入向导 | §1.2 | 🟡 上传有 | v1.5 (1 周) |
| 2 | 多模态文档支持 | §1.2 | 🟡 PDF/Word | v1.5 (2 周) |
| 3 | 跨知识库联合检索 | §1.4 | 🟡 RAG.search 支持 | v1.5 (1 周) |

#### X.2.3 P2 - 体验完善

| 1 | 知识图谱构建 | §1.4 | ❌ | v2.0+ |
| 2 | 智能问答融合 | §1.4 | 🟡 检索有 | v2.0+ |

### X.3 API 接口概要（Q2=B 归属更新）

> **触发决策**: 2026-07-27 与用户确认 Q2=B，"合并到现有 TECH 服务"
>
> **本模块 API 前缀**: /v1/kb + /v1/rag
> **归属后端服务**: TECH-RAG
>
...
| /v1/kb | 5 | TECH-RAG | KnowledgeBaseController/DocumentController | P0 |
| /v1/rag/search | 1 | TECH-RAG | SearchController | P0 |

#### X.3.2 前端代码位置

- 独立 app: `metaplatform-frontend/apps/kb/src/api/`
- portal 聚合: `metaplatform-frontend/apps/portal/src/pages/kb/`
...
- 总页面数：见盘点报告附录 B
- API 模块数：见盘点报告 §五
- 实现状态：详见盘点报告 §五.APP-KB
- 覆盖率：70%

### X.5 前端代码 → PRD 章节映射

按盘点报告 §五.APP-KB 的页面/API 清单与本 PRD 章节的对应关系，明确"哪些页面/API 由前端哪个文件承载、哪些是 Mock 实现"。
...
**触发位置**:
- APP-KB 知识库列表页 → 知识库行「编辑」按钮
- 知识库详情页右上角「编辑」按钮

**前置条件**:
- 当前用户拥有 `kb.update` 权限
- 通常仅 DRAFT 状态可修改编码和模型

**操作流程**:
1. 点击「编辑」→ 弹出 Modal
2. 系统加载现有知识库（GET /v1/kb/knowledge-bases/{id}）
3. 用户可修改：
   | 字段 | 类型 | 必填 | 是否可改 | 说明 |
   |---|---|---|---|---|
   | 显示名 | string | 是 | 是 | 1-128 字符 |
   | 编码 | string | 否 | **否（PUBLISHED 状态）** | |
   | 类型 | radio | 否 | **否** | 创建后不可改 |
   | 描述 | string | 否 | 是 | 0-512 字符 |
   | 切片策略 | select | 否 | 是 | 修改后下次切片生效 |
   | Embedding 模型 | radio | 否 | **否** | 修改需重建索引 |
   | 向量库 | radio | 否 | **否** | |
   | 语言 | radio | 否 | 是 | |
   | 可见性 | radio | 是 | 是 | PRIVATE/ORG/TENANT/PUBLIC |
   | 标签 | tags | 否 | 是 | |
   | 消费员工 | multi-select | 否 | 是 | |

4. 如果修改了 Embedding 模型或向量库：
   - 提示「需要重建索引，可能耗时较长」
   - 用户确认后异步重建
5. 点击「保存」→ PUT /v1/kb/knowledge-bases/{id}
...
**关联 API**:
- GET /v1/kb/knowledge-bases/{id}
- PUT /v1/kb/knowledge-bases/{id}
...
**前置条件**:
- 当前用户拥有 `kb.delete` 权限
- 知识库必须先停用（status = DISABLED）

**操作流程**:
1. 点击「删除」→ 弹出确认 Modal：
   - 提示「将删除知识库及其所有文档、切片、向量数据，不可恢复」
   - 要求输入知识库名称进行二次确认
2. 用户输入正确的名称
3. 点击「确认删除」→ DELETE /v1/kb/knowledge-bases/{id}
4. 后端处理：
   - 删除所有 Document
   - 删除所有 Chunk
   - 删除所有 Embedding
...
**结果反馈**:
- 成功：toast "知识库已删除，30 天后清理"
- 失败：toast 显示具体错误

**关联 API**:
- DELETE /v1/kb/knowledge-bases/{id}
...
**触发位置**:
- 知识库详情页 → 文档列表 → 文档行「处理」按钮
- 文档状态为 UPLOADING/PARSING/CHUNKING/EMBEDDING/INDEXING 时自动重试

**前置条件**:
- 文档已上传（status != READY）
- 用户拥有 `document.process` 权限

**操作流程**:
1. 点击「处理」→ 弹出执行配置 Modal：
   | 字段 | 类型 | 必填 | 默认 | 说明 |
   |---|---|---|---|---|
   | 处理步骤 | checkbox | 是 | 全选 | 解析/切片/Embedding/索引 |
   | 切片策略 | select | 是 | 知识库默认 | |
   | 失败重试 | number | 否 | 3 | 0-5 |
   | 异步执行 | boolean | 是 | true | false 时同步等待 |
2. 点击「开始处理」→ POST /v1/kb/documents/{id}/process
3. 状态机：
   ```
   UPLOADING → PARSING → CHUNKING → EMBEDDING → INDEXING → READY
                                                            ↘ FAILED
   ```
4. 实时显示状态（轮询或 WebSocket）
5. 失败时可点击「重试」→ 重新处理

**结果反馈**:
- 处理中：实时进度
- 成功：toast "处理完成，X 切片已生成"
- 失败：toast 显示错误，文档状态变 FAILED

**业务规则**:
- 单文档处理失败可重试 3 次
- 处理过程记录 ChunkingJob，可追踪
- 文档支持热更新（修改后重新切片）

**关联 API**:
- POST /v1/kb/documents/{id}/process
- GET /v1/kb/documents/{id}/status
...
| 字段 | 类型 | 必填 | 默认 | 校验 | 说明 |
|---|---|---|---|---|---|
| Top K | slider | 是 | 5 | 1-50 | 返回结果数 |
| Score 阈值 | slider | 是 | 0.6 | 0-1 | 低于此分数不返回 |
| 检索模式 | radio | 是 | "VECTOR" | VECTOR/KEYWORD/HYBRID | 混合检索 |
| Rerank | boolean | 是 | false | - | 开启后更精准但慢 |
| Rerank 模型 | select | 条件 | - | Rerank=true 时必填 | |

#### B. 过滤条件
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| 文件类型 | multi-checkbox | 否 | PDF/DOCX/... |
| 标签 | multi-select | 否 | 知识库内标签 |
| 元数据 | object | 否 | 自定义元数据过滤 |
| 日期范围 | daterange | 否 | from/to |

#### C. 查询改写
| 字段 | 类型 | 默认 | 说明 |
|---|---|---|---|
| 启用 Query 改写 | boolean | true | 用 AI 改写用户 query |
| 改写模型 | select | - | 默认 LLM |
| 多 Query 扩展 | boolean | false | 一次生成多个 query 并合并结果 |

#### D. 保存为模板
1. 用户配置好参数后
2. 点击「保存为模板」
3. 输入模板名
4. POST /v1/rag/templates（新建）
5. 下次直接选择模板
...
2. 弹出反馈 Modal（如选评论）：
   | 字段 | 类型 | 必填 | 说明 |
   |---|---|---|---|
   | 类型 | radio | 是 | RELEVANT/IRRELEVANT/PARTIAL |
   | 评论 | textarea | 否 | 0-500 字符 |
   | 标签 | tags | 否 | 多选，如"答非所问" |

3. 点击「提交」→ POST /v1/rag/feedback
4. 后端记录反馈到 SearchLog
...
**业务规则**:
- 反馈用于优化 Rerank 模型
- 质量评分 < 0.5 的切片自动标记为待审核
- 审核拒绝的切片不参与检索

**关联 API**:
- POST /v1/rag/feedback
- GET /v1/rag/feedback（汇总）
...
**关联 API**:
- POST /v1/rag/search?crossLanguage=true
