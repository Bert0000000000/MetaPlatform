...
## 10. 前端组件技术规范（v1.2 新增 2026-07-28）

> **触发决策**: 2026-07-28 前后端并行开发，前端已实现 5 个大数据 View 组件，补充技术规范

### 10.1 组件架构总览

```
pages/ontology/
├── OntologyDatacenterPage.tsx       (主页面 + AI Assistant + 9 SubTab + 4 inline View)
└── components/
    ├── common.ts                     (共享格式化函数)
    ├── BigDataSourceView.tsx         (大数据源 CRUD)
    ├── CDCView.tsx                   (CDC 实时同步)
    ├── ETLView.tsx                   (大数据 ETL 任务)
    ├── SchedulerView.tsx             (任务调度中心)
    └── MetricView.tsx                (数据指标平台)
```
...
```tsx
const stats = [
  { label: '总任务', value: data.length },
  { label: '运行中', value: data.filter(d => d.status === 'RUNNING').length },
];
return <div className="v-card" style={{padding: 12}}>...</div>;
```

#### 10.2.3 Modal 流程
```tsx
const [showCreate, setShowCreate] = useState(false);
{showCreate && <CreateXxxDialog onClose={() => setShowCreate(false)} onSuccess={() => {setShowCreate(false); load();}} />}
```

### 10.3 API 客户端规范 (v1.2 新增)

`api/ontology-bigdata.ts` 提供统一 API 客户端：
- 独立 axios 实例 (baseURL=/api/v1, timeout=30s)
- 响应拦截器自动解包 ApiResponse
- META 常量 re-export (SOURCE_TYPE_META, ETL_MODE_META, METRIC_TYPE_META)
- 25+ API 函数, 全部 mock fallback

### 10.4 META 常量模式 (v1.2 新增)

```typescript
// types/ontology-bigdata.ts
export const SOURCE_TYPE_META = {
  HIVE: { label: 'Hive', color: '#f59e0b', icon: '🐝' },
  // ...
};

// api/ontology-bigdata.ts 中 re-export
...
<ErrorBoundary fallback={<SubErrorFallback name={tabName} />}>
  {subView}
</ErrorBoundary>
```
...
`components/common.ts` 提供：
- formatDuration(ms) -> "1.5s"/"1.5min"
- formatNumber(n) -> "1.50K"/"1.50M"/"1.50B"
- formatTimestamp(iso) -> 本地化时间

### 10.8 Mock 数据模式 (v1.2 新增)
...
### 10.9 Lucide React 图标 (v1.2 验证)

0.468.0 版本包含的图标（已确认可用）：
通用: Database, Plus, RefreshCw, Settings2, Trash2, Search, X, Loader2, AlertCircle, Play, Pause, Square, Clock
数据: BarChart3, TrendingUp, TrendingDown, GitBranch, Calculator, ArrowRight, Calendar, Bell
状态: CheckCircle2, AlertCircle, AlertTriangle, Info, HelpCircle

### 10.10 性能要求 (v1.2 新增)

| 操作 | P50 | P99 |
|---|---|---|
| 列表渲染 (10 行) | < 100ms | < 200ms |
| 列表渲染 (100 行) | < 500ms | < 1s |
| 搜索 (mock) | < 10ms | < 50ms |
| Modal 打开 | < 50ms | < 100ms |
| 卡片渲染 (4 卡) | < 30ms | < 80ms |
| 页面切换 (SubTab) | < 100ms | < 200ms |
...
|---|---|---|
| 单元 | Vitest | 70% |
| 组件 | @testing-library/react | 50% |
| E2E | Playwright | 关键流程 100% |
| 视觉 | Chromatic | PR 必跑 |
| 性能 | Lighthouse | P99 < 200ms |

### 10.12 CI/CD 流水线 (v1.2 新增)
...
## 11. Force-Directed 图形布局算法（v1.3 新增）

> **触发决策**: 2026-07-28 LineageFullView 升级，新增力导向布局

### 11.1 算法选择

**不引入 d3-force / react-force-graph**，自实现轻量级力导向布局，理由：
- 24 节点规模下 d3-force 过度工程化
- 减少打包体积（~200KB → 0KB）
- 完全控制交互细节
...
```
3 种力:
1. 库仑排斥 (Repulsion)   - 节点间相互排斥
2. 胡克弹簧 (Hooke Spring)  - 边连接的节点相互吸引
3. 中心引力 (Center Attract)  - 所有节点被拉向画布中心
```
...
| 参数 | 值 | 含义 |
|---|---|---|
| `repulsion` | 4500 | 排斥力常数 (距离平方反比) |
| `springLen` | 130 px | 弹簧自然长度 |
| `springK` | 0.04 | 弹簧刚度 |
| `centerK` | 0.012 | 中心引力常数 |
| `damping` | 0.85 | 每帧速度衰减 |
| `maxVel` | 8 px/帧 | 速度上限 |
| `iterations` | 350 | 总模拟步数 |

### 11.4 算法伪代码

```python
def runForceLayout(nodes, edges, width, height):
    pos = init_random_positions(nodes, width, height)
    for iter in 0..350:
        # 1. 排斥力
        for i, j in all_pairs(nodes):
            f = repulsion / dist(i, j)^2
            apply_force(i, j, f)
        # 2. 弹簧力
        for edge in edges:
            f = (dist - springLen) * springK
            apply_force(edge.from, edge.to, f)
        # 3. 中心引力
        for node in nodes:
            apply_force(node, center, centerK)
        # 4. 更新位置 (Verlet 积分)
        for node in nodes:
            v = node.v * damping  # 阻尼
            v = clamp(v, maxVel)
            node.x += v.x; node.y += v.y
    return pos
```
...
```typescript
// 用户拖拽节点后, 该节点位置覆盖力布局结果
const [forcePos, setForcePos] = useState({});
const effectivePos = useMemo(() => {
  return nodes.reduce((acc, n) => ({
    ...acc,
    [n.id]: forcePos[n.id] || forceLayout[n.id],
  }), {});
}, [forceLayout, forcePos]);
```

- 拖拽前: `effectivePos[n.id] = forceLayout[n.id]` (力布局)
- 拖拽后: `effectivePos[n.id] = forcePos[n.id]` (用户覆盖)
- 重置视图: `setForcePos({})` 清空覆盖, 回到纯力布局
...
| 元素 | 实现 |
|---|---|
| 节点 | `<g transform="translate(x,y)">` 包裹 `<circle>` + `<text>` |
| 边 | `<path d="M x1 y1 C cx1 cy1 cx2 cy2 x2 y2">` 贝塞尔曲线 |
| 缩放 | SVG `viewBox` + `transform: scale()` |
| 平移 | SVG `viewBox` 偏移 |
...
|---|---|
| 节点重叠 | 排斥力自动分离 |
| 超出画布 | 初始位置限定在中心 1/3 区域内 |
| 性能 | 350 步迭代后停止, 不再持续模拟 |
| 用户拖拽 | 覆盖力布局, 不再模拟该节点 |
