...
> v1.0 → v2.0 主要变更：根据设计稿刷新，章节结构稳定（7 大节）；MCP 注册基线由 Nacos 2.x 升级到 Nacos 3.0+；客户端 SDK 统一为 Spring AI Alibaba Nacos MCP Starter；新增 UI 设计基线附录。
...
| 维度 | 数量 | 说明 |
|---|---|---|
| PRD 需求总数 | 26 | 按功能点细粒度统计（覆盖 PRD 第 3 章 7 个功能域 + Dashboard 概览页 + 告警规则等子项） |
| ✅ 已实现 | 3 | 代码功能完整匹配 PRD |
| 🟡 部分实现 | 15 | 页面骨架在但深度不足 |
| ❌ 未实现 | 8 | PRD 需求在代码中无对应实现 |
| 🟠 Mock 临时 | 0 | 业务数据均通过真实 `/api/v1/mcp/*` axios 调用 |
| **覆盖率** | **60%** | 较 v1.0 评估 77% ↓17%（"页面骨架完整但深度不足"问题最严重） |

### v1.0 评估偏差说明

v1.0 规划文档按"功能域"评估为 77% 覆盖率，本次按 PRD 需求项细粒度核对发现每个功能域的"深度"普遍不足——大部分表单字段简化、缺少 PRD 要求的高级特性（版本管理、ABAC、断点调试、链路可视化、告警规则、矩阵视图等）。
...
|---|---|---|---|
| Dashboard | MCP 概览页（Server 状态/工具数/调用统计/Token/错误告警） | ❌ 未实现 | V12-03 |

> 当前路由 `/` 直接重定向到 `/tools`，无概览页。

#### P1 - v1.3 深度增强（集中补全字段与高级功能）

| REQ 编号 | 需求简述 | 当前状态 | 对应 Task |
|---|---|---|---|
| REQ-3.1.2 | ServerForm 字段补全（监听地址/端口/SSE端点/认证方式/超时/并发数） | 🟡 部分 | V13-01 |
| REQ-3.1.3 | ServerDetailPage 三 Tab 管理（Tools/Resources/Prompts）+ 添加/移除交互 | 🟡 部分 | V13-01 |
| REQ-3.1.4 | Server 重启按钮 + 连接状态面板 | 🟡 部分 | V13-01 |
| REQ-3.2.2/3.2.3 | Client 独立详情页 + 测试连接 + 工具发现 + 字段补全 | 🟡 部分 | V13-02 |
| REQ-3.3.3 | 工具版本管理（历史/对比/回滚/设为当前版本） | ❌ 未实现 | V13-03 |
| REQ-3.3.4 | 工具分类管理 CRUD（新增/编辑/删除/拖拽） | ❌ 未实现 | V13-03 |
| REQ-3.4.1/3.4.3 | 调试器三栏布局 + 原始报文/调用信息/历史记录 Tab + 断点/回放 | 🟡 部分 | V13-04 |
| REQ-3.5.1~3.5.4 | 调用审计 4 统计卡片 + 时间范围 + 多维筛选 + 导出 + Token/错误多维分析 + 告警规则 | 🟡 部分 | V13-05 |

#### P2 - v1.3 深度增强（续）

| REQ 编号 | 需求简述 | 当前状态 | 对应 Task |
|---|---|---|---|
| REQ-3.6.2~3.6.4 | ABAC 条件表达式编辑器 + 矩阵视图 + 权限模拟 | 🟡 部分 | V13-06 |
| REQ-3.7.1/3.7.3 | IDE 配置模板自动生成（5 种 IDE）+ 连接状态监控面板 | 🟡 部分 | V13-07 |

### 迭代版本映射

| 版本 | 主线 | 本模块 Task | 预估工作量 |
|---|---|---|---|
| v1.2 核心补齐 | 主线 2 | V12-03（概览页实现） | 2 人天 |
| v1.3 深度增强 | 主线 3 | V13-01~V13-07 | 16.5 人天 |
| v1.4 体验完善 | 主线 4 | V14-09（外部应用对接完整能力） | 1.5 人天 |
...
> **v2.0 重要变更（2026-07-22）**：MCP 注册基线由 Nacos 2.x 升级到 **Nacos 3.0+**，详情见附录 B。客户端 SDK 由自研统一为 **Spring AI Alibaba Nacos MCP Starter**。
