...
> **版本**: v2.2 | **日期**: 2026-07-27 | **模块**: APP-MCPHUB | **状态**: 正式版
>
> **vv2.1 → vv2.2 主要变更**：
> 1. **范围对齐前端实现**：根据"以独立 APP 为主"策略，PRD 描述范围与前端代码 1:1 对齐
> 2. **API 接口按 Q2=B 决策归属**：所有 /v1/mcp + /v1/iam (策略) 端点归到 TECH-MCP + TECH-IAM（策略）
...
1. **范围对齐前端实现**：根据"以独立 APP 为主"策略，PRD 描述范围与前端代码 1:1 对齐
2. **API 接口按 Q2=B 归属**：本模块所有 API 端点统一归到 **TECH-MCP + TECH-IAM（策略）**
...
| 1 | 告警规则独立页 | §3.5 调用审计 | 🟡 alert-rules API 有 | v1.4 (1 周) |
| 2 | 调试器三栏布局（请求/响应/上下文） | §3.4 调试器 | 🟡 DebuggerPage 有 | v1.4 (1 周) |
| 3 | ABAC 矩阵视图深度 | §3.6.3 | 🟡 PermissionMatrixPage 有 | v1.4 (1 周) |

#### X.2.2 P1 - 增强功能

| 1 | 工具 JSON Schema 可视化编辑器 | §3.3.3 | 🟡 ToolForm 有 | v1.5 (2 周) |
| 2 | 工具版本管理 | §3.3.4 | ❌ | v1.5 (2 周) |
| 3 | 调试断点/回放 | §3.4 | ❌ | v1.5 (2 周) |
| 4 | IDE 配置模板（Cursor/Cline/Copilot） | §3.7.1 | 🟡 IdeConfigPage 有 | v1.5 (1 周) |
| 5 | 连接状态监控 | §3.7.3 | 🟡 ConnectionMonitorPage 有 | v1.5 (1 周) |

#### X.2.3 P2 - 体验完善

| 1 | 自动发现机制 | 隐含 | ❌ | v2.0+ |
| 2 | ABAC 策略模板 | §3.6 | 🟡 框架 | v2.0+ |
| 3 | 协作审计深度 | §3.7.6 | 🟡 CollaborationAuditPage 有 | v2.0+ |

### X.3 API 接口概要（Q2=B 归属更新）

> **触发决策**: 2026-07-27 与用户确认 Q2=B，"合并到现有 TECH 服务"
>
> **本模块 API 前缀**: /v1/mcp + /v1/iam (策略)
> **归属后端服务**: TECH-MCP + TECH-IAM（策略）
>
...
| /v1/mcp | 60+ | TECH-MCP |  | P0 |
| /v1/iam (策略部分) | 7 | TECH-IAM | PolicyController | P0 |

#### X.3.2 前端代码位置

- 独立 app: `metaplatform-frontend/apps/mcphub/src/api/`
- portal 聚合: `metaplatform-frontend/apps/portal/src/pages/mcphub/`
...
- 总页面数：见盘点报告附录 B
- API 模块数：见盘点报告 §五
- 实现状态：详见盘点报告 §五.APP-MCPHUB
- 覆盖率：60%

### X.5 前端代码 → PRD 章节映射

按盘点报告 §五.APP-MCPHUB 的页面/API 清单与本 PRD 章节的对应关系，明确"哪些页面/API 由前端哪个文件承载、哪些是 Mock 实现"。
...
> **触发决策**: 2026-07-27 P0 完成后继续 P1（编辑/配置/复杂交互类）

---

### 9.23 【编辑 MCP Server】操作

**触发位置**:
- APP-MCPHUB Server 列表页 → Server 行的「编辑」按钮
- Server 详情页（ServerDetailPage）右上角「编辑」按钮

**前置条件**:
- 当前用户拥有 `server.update` 权限
- Server 状态允许编辑（通常 DRAFT/STOPPED/ERROR 可编辑，RUNNING 需先停止）

**操作流程**:
1. 点击「编辑」→ 弹出 Modal（与新建 Server 类似）
2. 系统加载现有配置（GET /v1/mcp/servers/{id}）
3. 用户修改：
   | 字段 | 类型 | 必填 | 校验 | 说明 |
   |---|---|---|---|---|
   | 名称 | string | 是 | 1-64 字符 | 可修改 |
   | 编码 | string | 否 | 只读 | 创建后不可改 |
   | 描述 | string | 否 | 0-1024 字符 | |
   | 端点 URL | string | 是 | URL 格式 | |
   | 命令 | string | 条件 | STDIO 时必填 | |
   | 参数 | array | 否 | - | |
   | 环境变量 | json | 否 | - | 加密存储 |
   | 认证类型 | radio | 是 | - | 修改后需重新测试连接 |
   | 认证配置 | json | 条件 | 启用认证时必填 | 加密 |
   | 健康检查 URL | string | 否 | URL | |
   | 健康检查间隔 | number | 否 | 60 | 10-3600 秒 |
   | 超时 | number | 否 | 30 | 1-300 秒 |
   | 最大连接数 | number | 否 | 100 | 1-10000 |
   | 标签 | tags | 否 | - | |

4. 点击「保存」→ 触发后端校验：
   - 如果修改了端点或认证：要求重新测试连接
   - 如果 Server 正在运行：提示「需要重启才能生效」
5. 调用 PUT /v1/mcp/servers/{id}
6. 更新成功后：
   - 提示「已更新，是否重启 Server？」
   - 是 → 调用 POST /v1/mcp/servers/{id}/restart
   - 否 → 仅保存配置，下次启动生效
...
- 失败：
  - 重名校验失败：toast "名称重复"
  - 连接测试失败：toast "无法连接目标地址"
  - 状态不允许：toast "RUNNING 状态需先停止"
...
### 9.24 【删除 MCP Server】操作

**触发位置**:
- Server 列表页 → Server 行的「删除」按钮
- Server 详情页 → 「删除」按钮

**前置条件**:
- 当前用户拥有 `server.delete` 权限
- Server 必须先停止（status != RUNNING）
- 必须先卸载所有关联 Tool

**操作流程**:
1. 点击「删除」→ 弹出确认 Modal：
   - 提示「此操作将级联删除 Server 下所有 Tool/Resource/Prompt 配置，不可恢复」
   - 要求输入 Server 名称进行二次确认
2. 用户输入正确的 Server 名称
3. 点击「确认删除」→ 调用 DELETE /v1/mcp/servers/{id}
4. 后端处理：
   - 删除所有 Tool 配置
   - 删除所有 Resource 配置
   - 删除所有 Prompt 配置
...
**结果反馈**:
- 成功：toast "Server 已删除"，列表刷新
- 失败：
  - 名称不匹配：toast "名称输入错误"
  - Server 正在运行：toast "请先停止 Server"
  - 还有 Client 连接：toast "请先断开所有 Client"
...
**触发位置**:
- Tool 列表页（ToolListPage）→ Tool 行的「编辑」按钮
- Tool 详情页（ToolDetailPage）→ 「编辑」按钮

**前置条件**:
- 当前用户拥有 `tool.update` 权限
- Tool 状态允许编辑（DRAFT 状态可任意编辑，PUBLISHED 状态只能修改 description）

**操作流程**:
1. 点击「编辑」→ 弹出 Modal
2. 系统加载现有 Tool（GET /v1/mcp/tools/{id}）
3. 用户可修改：
   | 字段 | 类型 | 必填 | 是否可改 | 说明 |
   |---|---|---|---|---|
   | 名称 | string | 是 | 是 | |
   ...
   | 描述 | string | 是 | 是 | |
   | 输入 Schema | json | 是 | **是（创建新版本）** | 修改后自动创建 v2 版本 |
   | 输出 Schema | json | 否 | 是 | |
   | 示例 | json | 否 | 是 | |
   | 分类 | select | 否 | 是 | |
   | 启用 | boolean | 是 | 是 | |
...
4. **重要**：如果修改了 inputSchema/outputSchema：
   - 系统自动创建新版本（v2）
   - 现有调用方使用的还是 v1
   - 用户可选择：
     - 「发布新版本」：v1 → DEPRECATED，v2 → CURRENT
     - 「保存为草稿」：v2 仍为 DRAFT
5. 点击「保存」→ 调用 PUT /v1/mcp/tools/{id}

**结果反馈**:
- 成功：toast "Tool 已更新"+ 版本信息
- 失败：
  - Schema 验证失败：toast "JSON Schema 不合法"
  - 版本发布冲突：toast "请先处理已有 DRAFT 版本"

**关联 API**:
- GET /v1/mcp/tools/{id}
- PUT /v1/mcp/tools/{id}
- POST /v1/mcp/tools/versions（创建版本）
...
**触发位置**:
- APP-MCPHUB 调试器页（DebuggerPage，路径 `/debugger`）中央操作区
- 嵌入式调试器（任何 Tool 详情页的"测试"按钮）

**前置条件**:
- 至少 1 个已注册的 Tool
- 用户拥有 `debug.execute` 权限

**操作流程**:

#### 调试器布局（三栏）
- **左栏（20%）**：Tool/Resource/Prompt 选择树
  - 按 Server 分组
  - 显示已注册的 Tool
  - 支持搜索
- **中栏（40%）**：参数配置区
  - 根据 inputSchema 自动渲染表单
  - 支持嵌套对象和数组
- **右栏（40%）**：执行结果区
  - 实时显示 stdout/stderr
  - 显示返回结果（JSON 高亮）
...
#### 操作步骤
1. 用户在左栏选择 Tool
2. 系统加载 Tool 的 inputSchema
3. 中栏自动渲染参数表单
4. 用户填写参数：
   - 必填字段标红
   - 复杂类型（Object/Array）有 JSON 编辑器
   - 支持「示例数据」按钮自动填充
5. 可选配置：
   | 字段 | 类型 | 默认 | 说明 |
   |---|---|---|---|
   | 超时 | number | Tool 默认 | 毫秒 |
   | dryRun | boolean | false | true 时不实际执行 |
   | captureTrace | boolean | true | 是否记录调用链 |
6. 点击「▶ 执行」
7. 右侧实时显示结果：
   - 加载中：转圈
   - 成功：返回结果 + 耗时 + Token 消耗
   - 失败：错误信息 + 错误码
8. 结果可保存为「测试用例」（供后续回归测试）
...
**关联 API**:
- POST /v1/mcp/debug/execute
- POST /v1/mcp/debug/history
- POST /v1/mcp/debug/compare（对比两次执行）
...
**操作流程**:
1. 用户选择两个调试会话（用 Checkbox）
2. 点击「📊 对比」→ 弹出对比 Modal
3. 系统加载两次执行的详情
4. 展示对比视图：
   | 维度 | 执行 1 | 执行 2 | 差异 |
   |---|---|---|---|
   | 时间 | 2026-07-27 10:00 | 2026-07-27 10:05 | +5min |
   | 输入 | {...} | {...} | fieldA: "old" → "new" |
   | 输出 | {...} | {...} | fieldB 变化 |
   | 耗时 | 234ms | 156ms | -78ms |
   | Token | 150 | 120 | -30 |
   | 成本 | $0.003 | $0.0024 | -$0.0006 |
   | 状态 | SUCCESS | FAILED | 不同 |
   | 错误码 | - | 5001 | 出现错误 |
...
**关联 API**:
- POST /v1/mcp/debug/compare
- GET /v1/mcp/debug/history
...
**触发位置**:
- APP-MCPHUB 告警规则页（`/alert-rules`）→「+ 新建规则」
- Server/Tool 详情页 → 「告警」Tab → 「+ 新建规则」

**前置条件**:
- 当前用户拥有 `alertRule.create` 权限
- 至少 1 个 Server 或 Tool

**操作流程**:
1. 点击「+ 新建规则」→ 弹出向导 Drawer（2 步）

#### 步骤 1/2：触发条件
1. 用户选择监控目标：
   | 字段 | 类型 | 必填 | 校验 | 说明 |
   |---|---|---|---|---|
   | 监控目标类型 | radio | 是 | TOOL/SERVER | |
   | 监控目标 | select | 是 | 已注册对象 | 列出所有 Tool/Server |
   | 指标 | select | 是 | ERROR_RATE/LATENCY/CALL_VOLUME/TOKEN_USAGE/COST/CONNECTION_COUNT | |
   | 条件 | string | 是 | DSL 语法 | 如 `> 5% 持续 5min` |
   | 严重度 | radio | 是 | LOW/MEDIUM/HIGH/CRITICAL | |

2. **条件 DSL 示例**：
   - `> 5%` （错误率超过 5%）
   - `> 5% 持续 5min` （持续 5 分钟超过 5%）
   - `P99 > 5s 持续 5min` （P99 延迟持续 5 分钟超过 5s）
   - `> 1000/小时` （每小时超过 1000 次）

#### 步骤 2/2：通知配置
1. 用户配置：
   | 字段 | 类型 | 必填 | 校验 | 说明 |
   |---|---|---|---|---|
   | 通知渠道 | multi-checkbox | 是 | 至少 1 个 | 站内/邮件/短信/IM/Webhook |
   | 通知对象 | multi-select | 是 | 至少 1 个 | 用户/角色/部门 |
   | 冷却时间 | number | 否 | 300 | 60-3600 秒 |
   | 启用 | boolean | 是 | true | |
   | 通知模板 | select | 否 | - | 已有模板 |

2. 点击「创建」→ POST /v1/mcp/alert-rules

**结果反馈**:
- 成功：toast "告警规则创建成功"
- 失败：toast 显示具体错误（如 DSL 解析失败）

**业务规则**:
- 告警触发后进入冷却期，避免重复触发
- CRITICAL 告警自动通知 on-call
- 告警恢复后自动标记 AUTO_RESOLVED

**关联 API**:
- POST /v1/mcp/alert-rules
- GET /v1/mcp/alert-rules
...
**触发位置**:
- APP-MCPHUB 协作审计页（`/collaborations`）

**前置条件**:
- 当前用户拥有 `audit.read` 权限

**操作流程**:
1. 进入协作审计页
2. 系统默认加载最近 7 天的审计日志（GET /v1/mcp/collaborations/logs）
3. 用户可配置过滤条件：
   | 字段 | 类型 | 必填 | 说明 |
   |---|---|---|---|
   | 时间范围 | daterange | 是 | 默认最近 7 天 |
   | 协作类型 | multi-select | 否 | SERVER/CLIENT/TOOL |
   | 状态 | multi-select | 否 | SUCCESS/FAILED/TIMEOUT |
   | 关键词 | string | 否 | 搜索 Tool 名/IP/错误信息 |
   | 参与方 | select | 否 | Server/Client/User |

4. 点击「搜索」→ 展示审计列表
5. 列表显示：
   | 时间 | 参与方 | 工具 | 状态 | 耗时 | Token | 成本 | 操作 |
   |---|---|---|---|---|---|---|---|
   | 10:00:01 | Server A → Client B | search-doc | ✅ SUCCESS | 234ms | 150 | $0.003 | [详情] |
...
**导出**:
- 「导出 CSV」→ POST /v1/mcp/audit/export
- 「导出 Excel」→ POST /v1/mcp/audit/export
- 「按天/月聚合」→ GET /v1/mcp/audit/analytics

**关联 API**:
- GET /v1/mcp/collaborations/logs
- POST /v1/mcp/audit/export
- GET /v1/mcp/audit/analytics
- GET /v1/mcp/audit/statistics
...
**触发位置**:
- APP-MCPHUB 权限策略页（`/policies`）→「+ 新建策略」→ 选择「从模板创建」

**前置条件**:
- 当前用户拥有 `policy.create` 权限
- 系统已预置策略模板
...
| 模板 | 适用场景 | 示例 |
|---|---|---|
| 基于角色 | 按角色控制 | 仅 ADMIN 可调用 delete |
| 基于组织 | 按组织控制 | 财务部用户访问财务数据 |
| 基于时间 | 工作时间限制 | 仅工作时间 9:00-18:00 可访问 |
| 基于 IP | IP 白名单 | 仅内网 IP 可访问 |
| 基于设备 | 设备类型 | 仅 PC 可访问，移动端禁止 |
| 数据敏感度 | 数据分级 | 公开/内部/机密/绝密 |
| 操作风险 | 风险等级 | 高风险操作需 MFA |
| 自定义 | 完全自定义 | 任意条件组合 |
...
3. 用户修改具体参数：
   | 字段 | 类型 | 必填 | 说明 |
   |---|---|---|---|
   | 策略名 | string | 是 | 1-128 字符 |
   | 描述 | string | 否 | 0-512 字符 |
   | 目标资源 | select | 是 | 已有 Tool/Server |
   | 效果 | radio | 是 | ALLOW/DENY |
   | 条件 | json | 是 | DSL 表达式 |
   | 优先级 | number | 否 | 0-1000 |
...
{
  "allOf": [
    { "field": "user.orgId", "operator": "equals", "value": "finance" },
    { "field": "time.hour", "operator": "between", "value": [9, 18] },
    { "field": "request.ip", "operator": "in", "value": ["10.0.0.0/8"] }
  ]
}
...
{
  "allOf": [
    { "field": "resource.sensitivity", "operator": "lte", "value": 3 },
    { "field": "user.clearance", "operator": "gte", "value": 3 }
  ]
}
```
...
2. 选择测试用户
3. 系统模拟评估
4. 显示「ALLOW」或「DENY」+ 评估链路

**结果反馈**:
- 应用成功：toast "策略已创建，立即生效"
- 测试显示 ALLOW：toast "✓ 通过"
- 测试显示 DENY：toast "✗ 拒绝" + 拒绝原因
...
- POST /v1/iam/policies/evaluate（测试）
- GET /v1/iam/policies/condition-syntax（DSL 语法参考）
...
**触发位置**:
- APP-MCPHUB 调试器（DebuggerPage）→ 工具栏「🔍 断点」

**前置条件**:
- 至少 1 个 Tool
- 用户拥有 `debug.breakpoint` 权限
...
3. 弹出断点配置：
   | 字段 | 类型 | 默认 | 说明 |
   |---|---|---|---|
   | 断点位置 | radio | "BEFORE" | BEFORE/AFTER/BOTH |
   | 条件 | string | - | 满足条件才中断 |
   | 命中次数 | number | 1 | 0=不限 |
   | 动作 | radio | "PAUSE" | PAUSE/SKIP/INSPECT |
4. 保存断点
5. 点击「▶ 执行」
6. 命中时：
   - 暂停
...
**关联 API**:
- POST /v1/mcp/debug/execute（with breakpoint 参数）
- POST /v1/mcp/debug/record
- GET /v1/mcp/debug/history
...
**触发位置**:
- APP-MCPHUB 告警规则页 → 规则行「📜 历史」按钮

**操作流程**:
1. 点击「📜 历史」
2. 展示该规则的所有触发记录：
   | 时间 | 指标值 | 阈值 | 严重度 | 状态 | 通知 | 持续时间 |
3. 点击记录查看详情：
   - 触发时刻的指标曲线
   - 通知渠道状态
   - 处理人
...
**关联 API**:
- GET /v1/mcp/alert-rules/{id}/history
- PATCH /v1/mcp/alert-records/{id}
...
**触发位置**:
- APP-MCPHUB 信任域页（`/trusts`）

**操作流程**:

#### A. 信任域列表
- 显示所有信任域
- 显示每个域的允许 Server 数量、当前活跃连接
...
2. 用户填写：
   ...
   |---|---|---|---|
   | 域名 | string | 是 | 1-64 字符 |
   | 证书 | upload | 是 | X.509 PEM |
   | 允许的 Server | multi-select | 是 | 至少 1 个 |
   | 信任级别 | radio | 是 | FULL/PARTIAL/DISTRUST |
   | 过期时间 | date | 否 | - |
3. 点击「确认」

**关联 API**:
- GET /v1/mcp/trust-domains
- POST /v1/mcp/trust-domains
- PUT /v1/mcp/trust-domains/{id}
- DELETE /v1/mcp/trust-domains/{id}
...
**触发位置**:
- APP-MCPHUB 外部应用对接页（`/integrations`）→「IDE 配置」Tab
...
#### C. 生成配置
1. 系统根据 IDE 模板生成配置 JSON/TOML/YAML
2. 显示预览
3. 一键下载

#### D. 应用配置
- Cursor：复制到 `~/.cursor/mcp.json`
- Cline：复制到 `~/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json`
- 显示操作指南
...
**关联 API**:
- POST /v1/mcp/ide-config/generate
