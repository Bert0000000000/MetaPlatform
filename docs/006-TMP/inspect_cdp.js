...
async function inspect(name, url) {
  ...
  const page = list.find(p => p.url === 'about:blank') || list[0];
  const wsUrl = page ? page.webSocketDebuggerUrl : null;
  ...
  await new Promise(r => { if (ws.readyState === 1) r(); else ws.onopen = r; });
...
(async () => {
  for (const [name, url] of urls) {
    try { await inspect(name, url); } catch (e) { console.error(name, e.message); }
  }
  process.exit(0);
})();
