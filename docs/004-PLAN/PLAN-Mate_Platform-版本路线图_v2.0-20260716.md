...
>
> 版本：v3.6（阶段三全量验证完成 - 15 后端模块 + 7 前端模块 + 1205 测试 100% + 持久化迁移 + 限流接线修复）
>
> 日期：2026-07-17（最近更新：2026-07-17 21:00）
>
...
> - **v3.6（2026-07-17 21:00）路线图状态同步 + 持久化迁移 + 限流接线修复**：
>   - **InMemory 仓库迁移到 PostgreSQL**：TECH-LLMGW 6 个域（model/provider/prompt/quota/rate_limit/api_key）+ TECH-AGENT 6 个域（agent/task/conversation/tool/execution_step/api_key）。
>   - **LLMGW 限流接线修复**：chat/completions 入口调用 `check_and_increment`，stats() 返回真实计数。
>   - P3-DATA-04 补充 warehouse history 端点。
>   - **阶段三全部 Task 代码实现验证完毕**，约 100+ 个 Task 状态从 `[ ]` 更新为 `[x]`。
...
| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-DW-01 | 任务列表页 + 创建任务 | P2-DW-05 | `[x]` | 创建任务分配给数字员工 |
| P3-DW-02 | 任务详情页 + 实时监控 | P3-DW-01, P2-AGT-06 | `[x]` | SSE 实时展示任务执行进度 |
| P3-DW-03 | 执行轨迹（时间线 + 回放） | P3-DW-02 | `[x]` | 可回放 Agent 执行轨迹 |
| P3-DW-04 | 日志查看（Trace 链路） | P3-DW-02, P1-OBS-02 | `[x]` | 查看 trace_id 关联的全链路日志 |
| P3-DW-05 | 任务干预（暂停/恢复/取消/重试） | P3-DW-02 | `[x]` | 暂停任务后可恢复，取消后可重试 |
| P3-DW-06 🆕 | 效果评估 - 对话记录与回放（历史对话查看+执行回放） | P3-DW-03 | `[x]` | 可查看员工历史对话，回放执行过程 |
| P3-DW-07 🆕 | 效果评估 - 质量评分与报告（质量评分+评估报告+趋势分析） | P3-DW-06 | `[x]` | 对话质量可评分，生成评估报告 |
| P3-DW-08 🆕 | 多员工协作 - 协作创建与调度配置（创建协作+任务拆分+员工分配） | P3-DW-01, P3-SAI-07 | `[x]` | 创建多员工协作任务，自动拆分和分配 |
| P3-DW-09 🆕 | 多员工协作 - 协作执行与监控（并行执行+实时进度+异常处理） | P3-DW-08, P2-AGT-06 | `[x]` | 多员工并行执行，SSE 实时进度 |
| P3-DW-10 🆕 | 多员工协作 - 结果汇总与报告（结果聚合+冲突处理+汇总报告） | P3-DW-09 | `[x]` | 多员工结果自动汇总，生成协作报告 |
| P3-DW-11 🆕 | 员工管理补全 - 克隆/删除/版本历史/操作日志 | P2-DW-04 | `[x]` | 员工可克隆、删除，版本历史可查 |
| P3-DW-12 🆕 | A2A 外部协作前端（外部 Agent 发现+委派+结果展示） | P3-A2A-05 | `[x]` | 在 DW 界面发现外部 Agent 并委派任务 |

### P3-03. APP-SUPERAI（Phase 4-5: 任务编排 + 数字员工调度）— **极高风险收敛点**

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-SAI-01 | 任务分解（复杂问题→子任务列表） | P2-SAI-09, P2-AGT-04 | `[x]` | 输入复杂问题→自动拆分为可执行子任务 |
| P3-SAI-02 | 执行计划生成（子任务→执行顺序+并行策略） | P3-SAI-01 | `[x]` | 生成含串行/并行节点的执行计划 |
| P3-SAI-03 | 并行执行 + 执行监控 | P3-SAI-02 | `[x]` | 多子任务并行执行，实时展示进度 |
| P3-SAI-04 | 结果聚合 + 异常处理 | P3-SAI-03 | `[x]` | 聚合子任务结果，异常自动重试 |
| P3-SAI-05 | 任务模板 + 报告生成 | P3-SAI-04 | `[x]` | 保存常用任务为模板，生成执行报告 |
| P3-SAI-06 | 调度意图识别（专业领域→调度模式） | P3-SAI-04 | `[x]` | 准确率 ≥ 85% 识别需要调度的意图 |
| P3-SAI-07 | 任务拆分 + 数字员工匹配 | P3-SAI-06, P2-DW-03 | `[x]` | 子任务匹配到合适的数字员工 |
| P3-SAI-08 | 调度计划卡片（展示拆分+匹配结果） | P3-SAI-07 | `[x]` | 可视化展示调度计划，支持编辑 |
| P3-SAI-09 | 确认调度 + 调度执行面板（SSE ≤ 2s） | P3-SAI-08, P2-AGT-06 | `[x]` | 实时展示各数字员工执行状态 |
| P3-SAI-10 | 执行详情查看 + 异常处理 | P3-SAI-09 | `[x]` | 点击查看单个员工执行详情，异常可处理 |
| P3-SAI-11 | 结果汇总（整理所有员工反馈→最终回答） | P3-SAI-09 | `[x]` | 汇总多员工结果生成结构化最终回答 |
| P3-SAI-12 | 导出报告 + 调度模板 | P3-SAI-11 | `[x]` | 导出调度报告，保存为模板 |
| P3-SAI-13 | 手动选择数字员工 | P3-SAI-07 | `[x]` | 用户可手动指定数字员工 |
| P3-SAI-14 | A2A 跨系统协作（外部 Agent 发现+委派） | P3-SAI-09, P3-A2A-05 | `[x]` | 搜索外部 Agent→委托任务→获取结果 |
| P3-SAI-15 | 调度模式集成验证 | P3-SAI-14 | `[x]` | 端到端：专业问题→调度→多员工执行→汇总→最终回答 |

### P3-04. APP-MCPHUB（P0-P2: MCP 服务中心前端 + Resource/Prompt/审计，并行）

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-MCPHUB-01 | 前端脚手架 + 工具注册中心页面 | P2-MCP-03 | `[x]` | Tool 列表/注册/编辑/分类管理 |
| P3-MCPHUB-02 | MCP Server 管理页面 | P2-MCP-02 | `[x]` | Server 列表/创建/配置 Tools/启停 |
| P3-MCPHUB-03 | MCP 调试器（连接→选 Tool→填参→执行→看响应） | P3-MCPHUB-01, P2-MCP-05 | `[x]` | 在线调试 MCP Tool 调用 |
| P3-MCPHUB-04 | MCP Client 管理（添加连接/工具发现/同步） | P3-MCPHUB-02 | `[x]` | 连接第三方 MCP Server 并发现 Tools |
| P3-MCPHUB-05 | 权限控制（规则列表/创建规则） | P3-MCPHUB-01 | `[x]` | 配置用户-工具权限规则 |
| P3-MCPHUB-06 🆕 | Resource 配置页面（文档/架构资产资源 CRUD + 内容读取） | P3-MCPHUB-01 | `[x]` | Resource 可配置和管理，支持内容预览 |
| P3-MCPHUB-07 🆕 | Prompt 模板配置页面（角色模板 CRUD + 渲染预览） | P3-MCPHUB-01 | `[x]` | Prompt 模板可配置，支持变量渲染预览 |
| P3-MCPHUB-08 🆕 | 调用审计页面（调用列表/详情/Token 统计/错误/趋势分析） | P3-MCPHUB-03 | `[x]` | MCP 调用审计可查，Token 消耗和趋势可视化 |
| P3-MCPHUB-09 🆕 | 外部应用集成（API 密钥管理/集成文档/在线测试） | P3-MCPHUB-05 | `[x]` | 外部应用可通过 API Key 集成 MCP |

### P3-05. APP-ARCH（Phase 1-3: 业务+应用+数据+技术架构+治理，并行）

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-ARCH-01 | 前端脚手架 + 业务架构总览页 | P2-EA-02 | `[x]` | 能力地图卡片展示 |
| P3-ARCH-02 | 能力地图管理（CRUD + 层级调整） | P3-ARCH-01 | `[x]` | 能力节点 CRUD，拖拽调整层级 |
| P3-ARCH-03 | 能力可视化（AntV X6 树形布局） | P3-ARCH-02 | `[x]` | 树形图展示能力层级 |
| P3-ARCH-04 | 应用系统注册 + 应用-能力关联 | P3-ARCH-01 | `[x]` | 注册应用系统并关联到业务能力 |
| P3-ARCH-05 | 依赖关系管理（拓扑图可视化） | P3-ARCH-04 | `[x]` | 应用间依赖拓扑图 |
| P3-ARCH-06 🆕 | 价值流管理页面（CRUD + 阶段管理 + 可视化） | P3-ARCH-03 | `[x]` | 价值流 CRUD，阶段可配置，可视化展示 |
| P3-ARCH-07 🆕 | 业务流程管理页面（CRUD + 能力关联 + 流程图） | P3-ARCH-06 | `[x]` | 业务流程 CRUD，关联业务能力 |
| P3-ARCH-08 🆕 | 组织与角色管理页面（组织树 + 角色管理 + 分配） | P3-ARCH-01 | `[x]` | 组织架构和角色管理 |
| P3-ARCH-09 🆕 | 数据架构管理页面（数据域/数据实体/数据流/数据资产） | P3-ARCH-04 | `[x]` | 数据架构四大子域 CRUD + 可视化 |
| P3-ARCH-10 🆕 | 技术架构管理页面（技术栈/基础设施/技术标准） | P3-ARCH-04 | `[x]` | 技术架构三大子域 CRUD |
| P3-ARCH-11 🆕 | 架构治理 - 原则与标准管理（架构原则 CRUD + 标准管理） | P3-ARCH-01 | `[x]` | 架构原则和标准可配置管理 |
| P3-ARCH-12 🆕 | 架构治理 - 评审流程管理（评审 CRUD + 提交/批准/拒绝 + 评论） | P3-ARCH-11 | `[x]` | 评审流程完整流转 |
| P3-ARCH-13 🆕 | 架构治理 - 技术债务与合规（技术债务 CRUD + 合规检查） | P3-ARCH-12 | `[x]` | 技术债务可跟踪，合规检查可执行 |
| P3-ARCH-14 🆕 | Ontology 联动（能力-概念映射可视化 + 影响分析） | P3-ARCH-05, P2-EA-04 | `[x]` | 架构资产与 Ontology 联动，变更影响分析 |

### P3-06. APP-DASHBOARD（Phase 2-6: 审批+指标+通知+历史交付材料+个性化，并行）

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-DASH-01 | 前端脚手架 + 基础工作台 | S-IAM-04, S-GW-02 | `[x]` | 首页布局+待办卡片+快捷入口+通知铃铛 |
| P3-DASH-02 | 审批任务卡片 + 跳转 APP-APPHUB | P3-DASH-01, P2-APPHUB-05 | `[x]` | 待办审批列表，点击跳转审批详情 |
| P3-DASH-03 | 数字员工状态卡片 | P3-DASH-01, P2-DW-04 | `[x]` | 数字员工运行状态实时展示 |
| P3-DASH-04 | WebSocket 实时推送 | P3-DASH-01, S-MSG-01 | `[x]` | 通知/待办实时推送到前端 |
| P3-DASH-05 | 指标看板（指标卡片 + 趋势图表 + 时间筛选） | P3-DASH-01 | `[x]` | 指标卡片展示，图表交互 |
| P3-DASH-06 | 通知中心（已读未读管理 + 通知设置） | P3-DASH-04 | `[x]` | 通知列表管理，已读未读切换 |
| P3-DASH-07 | 全局搜索 | P3-DASH-01 | `[x]` | 搜索应用/知识/本体/任务 |
| P3-DASH-08 🆕 | 历史交付材料 - 列表与详情（SuperAI 报告+DW 任务输出+调度摘要） | P3-DASH-01, P3-SAI-12 | `[x]` | 交付材料列表可查，详情可展示 |
| P3-DASH-09 🆕 | 历史交付材料 - 搜索与下载（全文搜索+多格式下载+通知） | P3-DASH-08 | `[x]` | 全文搜索交付材料，支持多格式下载 |
| P3-DASH-10 🆕 | 个性化设置（布局自定义拖拽+偏好设置+主题切换+会话管理+API Token） | P3-DASH-01 | `[x]` | 用户可自定义布局、主题、偏好，管理会话和 Token |

### P3-07. APP-ONTSTUDIO（Phase 2-5: 关系规则+数据中心+Action编排，并行）

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-ONTUI-01 | 关系类型管理 + 关系实例管理页面 | P1-ONTUI-05, P1-ONT-02 | `[x]` | 关系 CRUD 页面 |
| P3-ONTUI-02 | 规则管理页面（CRUD + 条件-动作配置 + 测试运行） | P3-ONTUI-01, P1-RULE-04 | `[x]` | 规则配置后可测试运行 |
| P3-ONTUI-03 | 版本管理（创建/发布/对比/回滚） | P3-ONTUI-01 | `[x]` | Ontology 版本对比和回滚 |
| P3-ONTUI-04 | 知识图谱可视化（AntV X6） | P1-ONT-05, P1-ONTUI-01 | `[x]` | 图谱浏览/节点展开/布局切换 |
| P3-ONTUI-05 | 数据源管理 + 数据映射页面 | S-DATA-03 | `[x]` | 连接数据源→字段映射→同步策略 |
| P3-ONTUI-06 | Action 定义 + 服务编排可视化 | P2-ACT-05 | `[x]` | 可视化编排 Action 节点 |
| P3-ONTUI-07 | 触发规则配置 + 执行监控 | P3-ONTUI-06, P2-ACT-09 | `[x]` | 配置触发规则，查看执行记录 |

### P3-08. APP-APPHUB（Phase 3-6: 页面设计器 + 版本管理 + 应用市场 + AI 辅助，并行）

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-APPHUB-01 | 发布校验集成（表单/流程校验 + 预览） | P2-APPHUB-05 | `[x]` | 校验通过后可发布，校验失败给出明确提示 |
| P3-APPHUB-02 | 流程测试联调（模拟审批全流程） | P3-APPHUB-01 | `[x]` | 模拟提交→审批流转→完成，全流程无报错 |
| P3-APPHUB-03 | 应用发布（多版本管理 + 回滚） | P3-APPHUB-01 | `[x]` | 应用可多版本发布，支持版本回滚 |
| P3-APPHUB-04 | 页面设计器（看板 + 数据表格 + 图表组件） | P3-APPHUB-01 | `[x]` | 拖拽组件构建看板页面，数据表格和图表可配置 |
| P3-APPHUB-05 | AI 辅助（AI 生成表单/流程配置） | P2-SAI-13, P3-APPHUB-04 | `[x]` | 描述需求→AI 生成表单/流程配置→可编辑导入 |
| P3-APPHUB-06 🆕 | 应用下线 + 应用详情页（完整应用生命周期管理） | P3-APPHUB-03 | `[x]` | 应用可下线，详情页展示完整配置和运行数据 |
| P3-APPHUB-07 🆕 | 版本管理详情（版本对比/差异可视化/回滚操作） | P3-APPHUB-03 | `[x]` | 版本差异可视化对比，支持一键回滚 |
| P3-APPHUB-08 🆕 | 应用市场 - 模板浏览/使用/分类/搜索/详情 | P3-APPHUB-03 | `[x]` | 模板市场可浏览、搜索、使用模板创建应用 |
| P3-APPHUB-09 🆕 | 表单全局设置 + 保存校验规则配置 | P1-APPHUB-06 | `[x]` | 表单全局参数可配置，自定义校验规则 |
| P3-APPHUB-10 🆕 | AI 辅助 - 流程配置生成入口（集成 SuperAI 到设计器） | P3-APPHUB-05 | `[x]` | 在流程设计器中直接调用 AI 生成 BPMN 草稿 |
| P3-APPHUB-11 🆕 | AI 辅助 - 代码片段生成与解释（集成 SuperAI 代码能力） | P2-SAI-15 | `[x]` | 在设计器中调用 AI 生成/解释代码片段 |
| P3-APPHUB-12 🆕 | AI 辅助 - 仪表盘/API 示例生成入口 | P2-SAI-17 | `[x]` | 在设计器中调用 AI 生成仪表盘配置和 API 示例 |

### P3-09. TECH-DATA（Phase 3: ETL + 数仓 + 数据目录 + 质量 + 监控）🆕

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-DATA-01 🆕 | ETL 任务管理 API（CRUD + 手动触发 + 运行列表/详情 + 日志） | S-DATA-03 | `[x]` | ETL 任务可创建/配置/执行，运行日志可查 |
| P3-DATA-02 🆕 | DBT 集成 API（项目/模型/编译/DAG 可视化 + 运行） | P3-DATA-01 | `[x]` | DBT 模型可编译和运行，DAG 可视化展示 |
| P3-DATA-03 🆕 | 湖表管理 API（Hudi/Iceberg CRUD + 入湖任务 + 分区管理） | S-DATA-03, S-MSG-02 | `[x]` | 湖表可创建管理，CDC 入湖任务可执行 |
| P3-DATA-04 🆕 | 数仓查询 API（查询/分层/表列表/物化视图/刷新/历史） | P3-DATA-02 | `[x]` | StarRocks 数仓可查询，物化视图可管理 |
| P3-DATA-05 🆕 | 数据资产目录 API（列表/详情/元数据/血缘/画像/搜索） | P3-DATA-01 | `[x]` | 数据资产可编目，血缘可追溯，画像可查 |
| P3-DATA-06 🆕 | 数据质量 API（规则 CRUD + 检查执行 + 报告 + 仪表板） | P3-DATA-05 | `[x]` | 数据质量规则可配置，检查报告可生成 |
| P3-DATA-07 🆕 | 监控告警 API（概览/SLA/告警/确认/日志） | P3-DATA-01, P1-OBS-04 | `[x]` | 数据管道监控告警，SLA 可查 |

### P3-10. TECH-IAM（Phase 3: 企业级认证 + 审计）🆕

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P1-IAM-08 🆕 | SSO 登录 API（OAuth2/OIDC/SAML 协议）+ MFA 多因素认证 | S-IAM-04 | `[x]` | 支持 SSO 第三方登录，MFA 可配置 |
| P1-IAM-09 🆕 | 审计日志 API（操作日志/登录日志/权限变更日志/查询/导出） | P1-IAM-04 | `[x]` | 全量操作审计日志可查，支持导出 |
| P1-IAM-10 🆕 | 用户高级管理 + 岗位管理 + 人员部门关联 API | S-IAM-06 | `[x]` | 用户可绑定岗位和部门，支持批量操作 |

### P3-11. TECH-GW（Phase 3: API 管理 + 审计 + 灰度）🆕

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P1-GW-03 🆕 | API 管理 CRUD API + 分组 + 版本（`/api/v1/gw/apis`） | S-GW-02 | `[x]` | API 可注册管理，支持分组和版本 |
| P1-GW-04 🆕 | OpenAPI 导出 + API 详情（Schema/参数/响应/示例） | P1-GW-03 | `[x]` | 可导出 OpenAPI 3.0 规范文档 |
| P1-GW-05 🆕 | 审计日志 API（请求日志/错误日志/延迟统计/调用链） | S-GW-02 | `[x]` | API 请求审计可查，延迟和错误可统计 |
| P1-GW-06 🆕 | 审计日志 API（慢请求/导出/告警规则） | P1-GW-05 | `[x]` | 慢请求可识别，日志可导出，告警可配置 |
| P1-GW-07 🆕 | 灰度发布 API（创建/列表/详情/规则配置/停止） | P1-GW-03 | `[x]` | 灰度发布规则可配置，流量按比例分流 |

### P3-12. TECH-OBS（Phase 3: 告警 + 仪表板 + SLO + Trace 完整）🆕

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P1-OBS-05 🆕 | Trace 查询 API（查询/详情/Span/拓扑/服务依赖图） | S-OBS-02 | `[x]` | 分布式 Trace 可查，服务依赖拓扑可视化 |
| P1-OBS-06 🆕 | 告警管理 API（规则 CRUD + 告警列表 + 通知渠道配置） | P1-OBS-04 | `[x]` | 告警规则可配置，告警通知可发送 |
| P1-OBS-07 🆕 | 告警管理 API（静默/恢复/历史/统计） | P1-OBS-06 | `[x]` | 告警可静默和恢复，历史可查 |
| P1-OBS-08 🆕 | 仪表板 API（CRUD + 面板组件 + 分享/导出） | P1-OBS-04 | `[x]` | 自定义仪表板可创建和分享 |
| P1-OBS-09 🆕 | 服务拓扑 API（拓扑图/健康状态/服务依赖/实时刷新） | P1-OBS-05 | `[x]` | 服务拓扑实时展示，健康状态可视化 |
| P1-OBS-10 🆕 | SLO 管理 API（CRUD + 错误预算/SLI/报告） | P1-OBS-04 | `[x]` | SLO 可配置，错误预算可追踪 |

### P3-13. TECH-RULE（Phase 3: 决策表 + 测试 + 监控）🆕

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P1-RULE-07 🆕 | 决策表 CRUD API + 列管理（`/api/v1/rule/decision-tables`） | P1-RULE-02 | `[x]` | 决策表可创建，条件列/动作列可配置 |
| P1-RULE-08 🆕 | 决策表规则编辑与验证 API（行编辑/批量导入/校验） | P1-RULE-07 | `[x]` | 决策表规则行可编辑，校验通过 |
| P1-RULE-09 🆕 | 规则测试 API（单条测试 + 批量执行 + 结果对比） | P1-RULE-02 | `[x]` | 输入测试数据→执行规则→返回匹配结果 |
| P1-RULE-10 🆕 | 测试用例管理 API（CRUD + 模拟执行 + 版本对比） | P1-RULE-09 | `[x]` | 测试用例可管理，支持回归测试 |
| P1-RULE-11 🆕 | 监控统计 API（执行统计/匹配率/错误追踪/历史/单规则统计） | P1-RULE-03 | `[x]` | 规则执行监控仪表盘，错误可追踪 |

### P3-14. TECH-EA（Phase 3: 完整 EA 架构资产）🆕

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-EA-01 🆕 | 价值流管理 API（CRUD + 阶段管理 + 能力关联 + 可视化） | P2-EA-02 | `[x]` | 价值流 CRUD，阶段可配置，关联业务能力 |
| P3-EA-02 🆕 | 业务流程管理 API（CRUD + 能力关联 + 流程图 + 版本） | P3-EA-01 | `[x]` | 业务流程 CRUD，关联业务能力 |
| P3-EA-03 🆕 | 应用管理 API（CRUD + 依赖管理 + 依赖图 + 技术栈） | P2-EA-01 | `[x]` | 应用系统可注册，依赖关系可视化管理 |
| P3-EA-04 🆕 | 数据架构 API（数据域/数据实体/数据流/数据资产 CRUD） | P2-EA-04 | `[x]` | 数据架构四大子域可管理 |
| P3-EA-05 🆕 | 技术架构 API（技术栈/基础设施/技术标准 CRUD） | P3-EA-03 | `[x]` | 技术架构三大子域可管理 |
| P3-EA-06 🆕 | 架构评审 API（CRUD + 提交/批准/拒绝 + 评论 + 附件） | P3-EA-03 | `[x]` | 评审流程完整流转，支持评论和附件 |
| P3-EA-07 🆕 | 技术债务 + 标准管理 + 影响分析 API | P3-EA-05, P2-EA-04 | `[x]` | 技术债务可跟踪，变更影响分析可执行 |
| P3-EA-08 🆕 | 本体映射 API（映射 CRUD + 一致性检查 + 自动同步） | P2-EA-04, P1-ONT-12 | `[x]` | EA 资产与 Ontology 概念映射，一致性可检查 |

### P3-15. TECH-MCP（Phase 3: Resource + Prompt + 审计）🆕

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-MCP-01 🆕 | Resource 管理 API（CRUD + 读取内容/搜索 + 关联 Ontology） | P2-MCP-02 | `[x]` | Resource 可注册管理，内容可读取和搜索 |
| P3-MCP-02 🆕 | Prompt 模板管理 API（CRUD + 渲染 + 变量 + 预览） | P2-MCP-02 | `[x]` | Prompt 模板可管理，变量渲染正确 |
| P3-MCP-03 🆕 | 审计统计 API（调用列表/详情/Token 统计/错误/趋势/导出） | P2-MCP-05 | `[x]` | MCP 调用全量审计，Token 消耗和趋势可视化 |

### P3-16. TECH-LLMGW（Phase 3: 成本报表 + 审计日志）🆕

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P3-LLMGW-01 🆕 | 成本报表 API（汇总/按用户/应用/模型/供应商/时间序列/导出） | S-LLMGW-03 | `[x]` | LLM 调用成本可统计，支持多维度分析和导出 |
| P3-LLMGW-02 🆕 | 审计日志 API（日志查询/详情/错误/延迟/按模型延迟/导出） | S-LLMGW-03 | `[x]` | LLM 调用全量审计，延迟和错误可追踪 |

### P3-17. TECH-WFE（Phase 3: 高级任务操作）🆕

| Task ID | 任务 | 依赖 | 状态 | 验收标准 |
|---|---|---|---|---|
| P1-WFE-10 🆕 | 高级任务操作 API（加签/委派/催办 + 历史查询 + 监控统计 + 事件订阅管理） | P1-WFE-09 | `[x]` | 任务支持加签/委派/催办，历史可查，事件可订阅 |
...
| 数字员工调度+效果评估+多员工协作 | P3-DW-12, P3-SAI-15 | `[x]` |（代码 + 构建通过，待 E2E） |
| APPHUB 全功能（页面设计器+版本+市场+AI） | P3-APPHUB-12 | `[x]` |（代码 + 构建通过，待 E2E） |
| ARCH 四大域+治理 | P3-ARCH-14 | `[x]` |（代码 + 构建通过，待 E2E） |
| DASHBOARD 全功能 | P3-DASH-10 | `[x]` |（代码 + 构建通过，待 E2E） |
| MCPHUB 完整（Resource+Prompt+审计） | P3-MCPHUB-09 | `[x]` |（代码 + 构建通过，待 E2E） |
| ONTSTUDIO 完整 | P3-ONTUI-07 | `[x]` |（代码 + 构建通过，待 E2E） |
| DATA 完整数据治理 | P3-DATA-07 | `[x]` |（代码 + 50 测试通过，待 E2E） |
| IAM 企业级（SSO+MFA+审计） | P1-IAM-10 | `[x]` |（代码 + 94 测试通过，待 E2E） |
| GW 完整（API管理+审计+灰度） | P1-GW-07 | `[x]` |（代码 + 65 测试通过，待 E2E） |
| OBS 完整（告警+仪表板+SLO+Trace） | P1-OBS-10 | `[x]` |（代码 + 112 测试通过，待 E2E） |
| RULE 进阶（决策表+测试+监控） | P1-RULE-11 | `[x]` |（代码 + 41 测试通过，待 E2E） |
| EA 完整 | P3-EA-08 | `[x]` |（代码 + 151 测试通过，待 E2E） |
| MCP 完整（Resource+Prompt+审计） | P3-MCP-03 | `[x]` |（代码 + 104 测试通过，待 E2E） |
| LLMGW 成本+审计 | P3-LLMGW-02 | `[x]` |（代码 + 112 测试通过，待 E2E） |
| WFE 高级任务操作 | P1-WFE-10 | `[x]` |（代码 + 65 测试通过，待 E2E） |
...
| v3.6 | 2026-07-17 21:00 | **路线图状态同步 + 持久化迁移 + 限流接线修复**：InMemory 仓库迁移到 PostgreSQL（TECH-LLMGW 6 域 + TECH-AGENT 6 域）；LLMGW 限流接线修复（chat/completions 入口调用 check_and_increment，stats() 返回真实计数）；P3-DATA-04 补充 warehouse history 端点；阶段三全部 Task 代码实现验证完毕，约 100+ 个 Task 状态从 `[ ]` 更新为 `[x]`；已知限制 #4/#5/#10 标记为已解除 |
...
| ~~4~~ | ~~TECH-AGENT 内存仓库~~ | ~~InMemoryAgentRepository 进程重启清空~~ | ✅ **已解除**（v3.6: 6 个域迁移到 PostgreSQL） |
| ~~5~~ | ~~数据持久化~~ | ~~所有 InMemory 仓库尚未迁移 PostgreSQL~~ | ✅ **已解除**（v3.6: LLMGW 6 域 + AGENT 6 域迁移完成） |
| ~~10~~ | ~~LLMGW 限流接线~~ | ~~chat/embedding 入口未调用 check_and_increment，stats() 硬编码 0~~ | ✅ **已修复**（v3.6: chat/completions 入口调用 check_and_increment，stats() 返回真实计数） |
