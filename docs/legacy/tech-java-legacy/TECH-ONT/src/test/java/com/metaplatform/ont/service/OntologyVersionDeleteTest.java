...
class OntologyVersionDeleteTest {
    ...
    void delete_existingVersion_succeeds() {
        OntologyVersionEntity existing = versionEntity(1, false);
        ...
        assertThatCode(() -> versionService.delete(existing.getVersionId()))
                .doesNotThrowAnyException();

        verify(versionRepository, times(1)).delete(eq(existing));
    }
    ...
    void delete_notFound_throwsException() {
        ...
        assertThatThrownBy(() -> versionService.delete(missingId))
    ...
    @Test
    void delete_currentVersion_throwsVersionConflict() {
        OntologyVersionEntity current = versionEntity(2, true);
        when(versionRepository.findByVersionIdAndTenantId(current.getVersionId(),
                TenantContext.DEFAULT_TENANT_ID)).thenReturn(Optional.of(current));

        assertThatThrownBy(() -> versionService.delete(current.getVersionId()))
                .isInstanceOf(OntException.class)
                .extracting(ex -> ((OntException) ex).getErrorCode())
                .isEqualTo(ErrorCode.VERSION_CONFLICT);

        verify(versionRepository, never()).delete(eq(current));
    }

    private OntologyVersionEntity versionEntity(int number, boolean current) {
