...
def main() -> int:
    ...
    r = http(
        "POST",
        f"{GW}/api/v1/ont/v2/shacl/validate",
        {
            "target_class": EMP,
            "individuals": [
                {
    ...
    step = r.get("conforms") is True and r.get("violations", [{}])[0].get("constraint") == "pattern"
    ok &= step
    print(
        f"[1] shacl severity=Warning conforms={r.get('conforms')} "
    ...
    r = http(
        ...
        f"{GW}/api/v1/ont/v2/shacl/validate",
        {
            ...
            "property_shapes": [
                {
                    "path": f"ont.{TENANT}.prop.name.v1",
                    "not_shape": {"path": f"ont.{TENANT}.prop.name.v1", "pattern": r"^\d+$"},
                }
            ],
        },
        token=tok,
    )
    step = r.get("conforms") is False and any(
        v.get("constraint") == "not" for v in r.get("violations", [])
    )
    ok &= step
    ...
    r = http(
        ...
        f"{GW}/api/v1/ont/v2/shacl/validate",
        {
            ...
            "property_shapes": [{"path": f"ont.{TENANT}.prop.name.v1", "language_in": ["en"]}],
        },
        token=tok,
    )
    step = not r.get("conforms") and any(
        v.get("constraint") == "languageIn" for v in r.get("violations", [])
    )
    ok &= step
            ...
            "individuals": [
                {
                    "rid": f"{EMP}#i4",
                    "class_rid": EMP,
                    "props": {f"ont.{TENANT}.prop.name.v1": ["EMP-1", "nope"]},
                }
            ],
            "property_shapes": [
                {
    ...
    step = not r.get("conforms") and any(
        v.get("constraint") == "qualifiedMinCount" for v in r.get("violations", [])
    )
    ok &= step
    print(f"[4] shacl qualifiedMinCount violation -> {'PASS' if step else 'FAIL'}")

    # ⑤ G33 alignment：显式 + 词汇 + 结构 → 传递单簇
    lcls, rcls = "ont.tenant-default.obj.g33-cust-a.v1", "ont.tenant-default.obj.g33-cust-b.v1"
    left = [
        {
            "rid": "a1",
            "class_rid": lcls,
            "props": {
                f"ont.{TENANT}.prop.name.v1": "n1",
                f"ont.{TENANT}.prop.email.v1": "shared@corp.io",
                f"ont.{TENANT}.prop.phone.v1": "555-0001",
            },
        },
        {"rid": "a2", "class_rid": lcls, "props": {f"ont.{TENANT}.prop.name.v1": "zeta"}},
    ]
    right = [
        {
            "rid": "b1",
            "class_rid": rcls,
            "props": {
                f"ont.{TENANT}.prop.title.v1": "w1",
                f"ont.{TENANT}.prop.email.v1": "shared@corp.io",
                f"ont.{TENANT}.prop.phone.v1": "555-0001",
            },
        },
        {"rid": "b2", "class_rid": rcls, "props": {f"ont.{TENANT}.prop.title.v1": "n1"}},
    ]
    r = http(
        "POST",
        f"{GW}/api/v1/ont/v2/alignment/run",
        {
            "left": left,
            "right": right,
            "explicit_pairs": [["a1", "b1"]],
        },
        token=tok,
    )
    ...
    ok &= step
    print(f"[5] alignment clusters={clusters} stats={stats} -> {'PASS' if step else 'FAIL'}")

    # ⑥ G33 merge-preview：字段并集 + 冲突 + 审计
    prop = lambda slug, type_id: {
    ...
    r = http(
        "POST",
        f"{GW}/api/v1/ont/v2/object-types/merge-preview",
        {
            "left": mk("g33-merge-a", [prop("id", "string"), prop("age", "integer")]),
            "right": mk(
                "g33-merge-b",
                [prop("id", "string"), prop("age", "string"), prop("email", "string")],
            ),
            "strategy": "keep_left",
        },
        token=tok,
    ...
    step = (
        audit.get("added") == [f"ont.{TENANT}.prop.email.v1"]
    ...
    ok &= step
    print(
        f"[6] merge-preview added={audit.get('added')} "
