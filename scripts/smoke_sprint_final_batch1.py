...
def type_def(slug: str, version: str = "v1", tenant: str = TENANT) -> dict:
    return {
        ...
        "properties": [{
            "rid": f"ont.{tenant}.prop.batch1-{slug}-id.v1", "type_id": "string",
...
def main() -> int:
    code, body = http("POST", f"{GW}/api/v1/iam/auth/login",
    ...
    code, _ = http("GET", f"{OT}/object-types",
    ...
    code2, b2 = http("POST", f"{OT}/object-types", type_def(slug_a, "v2"),
                     token=tok)
    check("dedup.slug_conflict_409", code1 == 200 and code2 == 409,
          f"v1={code1} v2={code2} "
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        codes = list(pool.map(
            lambda i: http("POST", f"{OT}/object-types",
                           type_def(slug_c, f"v{i + 1}"), token=tok)[0],
            range(4)))
    check("dedup.concurrent_create",
          codes.count(200) == 1 and codes.count(409) >= 1, f"codes={codes}")

    # B3: 跨租户写读隔离（GOVERN-06 修复后）
    code, _ = http("POST", f"{OT}/object-types",
                   type_def("dedupx", tenant="tenant-other"), token=tok)
    code_g, _ = http("GET", f"{OT}/object-types/ont.tenant-other.obj.batch1.dedupx.v1",
                     token=tok)
    check("dedup.cross_tenant_write_read_blocked", code == 403 and code_g == 404,
          f"write={code} read={code_g}")

    # B4: merge 回滚（6 段 rid）
    slug_s = f"dedups-{STAMP}"
    http("POST", f"{OT}/object-types", type_def(slug_s), token=tok)
    code, prop = http("POST", f"{OT}/object-types/propose-merge", {
        "source_rid": f"ont.{TENANT}.obj.batch1.{slug_s}.v1",
        "target_rid": f"ont.{TENANT}.obj.batch1.{slug_a}.v1",
        "similarity": 0.95, "impact_summary": "batch1 merge revert"},
        token=tok)
    pid = prop.get("proposal_id", "") if isinstance(prop, dict) else ""
    code_c, _ = http("POST", f"{OT}/proposals/{pid}/confirm", {}, token=tok,
                     headers={"Idempotency-Key": uuid.uuid4().hex})
    code_e, exe = http("POST", f"{OT}/proposals/{pid}/execute", {}, token=tok,
                       headers={"Idempotency-Key": uuid.uuid4().hex})
    code_r, rev = http("POST", f"{OT}/proposals/{pid}/revert", {}, token=tok,
                       headers={"Idempotency-Key": uuid.uuid4().hex})
    merged_ok = code_c == 200 and code_e == 200
    check("dedup.merge_revert", merged_ok and code_r == 200,
          f"confirm={code_c} exec={code_e} revert={code_r} "
          f"{json.dumps(rev, ensure_ascii=False)[:150] if isinstance(rev, dict) else ''}")

    # B5: precheck 相似度
    code, pre = http("POST", f"{OT}/object-types/precheck", {
        "name": slug_a, "slug": slug_a, "top_k": 3}, token=tok)
    cands = pre.get("candidates", []) if isinstance(pre, dict) else []
    check("dedup.precheck_candidates", code == 200 and len(cands) >= 1
          and cands[0].get("similarity", 0) > 0,
    ...
    td = {
        ...
        "properties": [
            {"rid": f"ont.{TENANT}.prop.batch1-{slug_m}-id.v1", "type_id": "string",
             "nullable": False, "primary_key": True, "title": "ID", "format": "string"},
            {"rid": f"ont.{TENANT}.prop.batch1-{slug_m}-name.v1", "type_id": "string",
             "nullable": True, "primary_key": False, "title": "Name", "format": "string"},
        ],
        "display_name": f"PropModel {STAMP}",
    }
    code, p1 = http("POST", f"{OT}/object-types/propose", {
        "type_def": td, "impact_summary": "batch1 proposal closure"}, token=tok)
    p1id = p1.get("proposal_id", "") if isinstance(p1, dict) else ""
    check("prop.model_type_pending", code == 200 and p1.get("status") == "pending",
          f"code={code}")

    code, _ = http("POST", f"{OT}/proposals/{p1id}/execute", {}, token=tok,
                   headers={"Idempotency-Key": uuid.uuid4().hex})
    check("prop.execute_unconfirmed_409", code == 409, f"code={code}")

    code, _ = http("POST", f"{OT}/proposals/{p1id}/confirm", {}, token=tok,
                   headers={"Idempotency-Key": uuid.uuid4().hex})
    ...
    check("prop.execute_registers_type", code == 200 and code_g == 200,
          f"exec={code} get={code_g}")
    ...
    code, _ = http("POST", f"{OT}/object-types/propose",
                   {"type_def": foreign_def, "impact_summary": "x"}, token=tok)
    ...
    code, p2 = http("POST", f"{OT}/classes/{td['rid']}/propose-instance",
                    {"props": {f"batch1-{slug_m}-id": f"i-{STAMP}",
                               f"ont.{TENANT}.prop.batch1-{slug_m}-name.v1": "alpha"},
                     "impact_summary": "instance revert check"}, token=tok)
    p2id = p2.get("proposal_id", "") if isinstance(p2, dict) else ""
    http("POST", f"{OT}/proposals/{p2id}/confirm", {}, token=tok,
         headers={"Idempotency-Key": uuid.uuid4().hex})
    code_e, exe2 = http("POST", f"{OT}/proposals/{p2id}/execute", {}, token=tok,
                        headers={"Idempotency-Key": uuid.uuid4().hex})
    ind_rid = exe2.get("individual_rid", "") if isinstance(exe2, dict) else ""
    if code_e != 200:
        print(f"  [diag] instance exec detail: "
              f"{json.dumps(exe2, ensure_ascii=False)[:200]}")
    code_g, _ = http("GET", f"{OT}/individuals/{ind_rid}", token=tok) if ind_rid else (0, "")
    code_r, rev2 = http("POST", f"{OT}/proposals/{p2id}/revert", {}, token=tok,
                        headers={"Idempotency-Key": uuid.uuid4().hex})
    code_g2, _ = http("GET", f"{OT}/individuals/{ind_rid}", token=tok) if ind_rid else (0, "")
    check("prop.instance_revert_rollback",
