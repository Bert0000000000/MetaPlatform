...
def login(hc: httpx.Client) -> str:
    r = hc.post(f"{GW}/api/v1/iam/auth/login", json={"username": "admin", "password": "admin123"})
...
def main() -> None:
    ...
    with httpx.Client(trust_env=False, timeout=60) as hc:
        ...
        def f_ont_list(c):
            return (
                c.get(f"{GW}/api/v1/ont/v2/object-types?limit=20", headers=auth).status_code == 200
            )
        ...
        def f_reasoning(c):
            return (
                c.post(f"{GW}/api/v1/ont/v2/reasoning/run", headers=auth, json=payload).status_code
                == 200
            )
        ...
        def f_badtoken(c):
            return (
                c.get(
                    f"{GW}/api/v1/ont/v2/object-types", headers={"Authorization": "Bearer bad"}
                ).status_code
                == 401
            )
    ...
    out = {"generated": time.strftime("%Y-%m-%dT%H:%M:%S"), "target": GW, "results": results}
    path = "/tmp/LOAD-TEST-V1.0.json"
    json.dump(out, open(path, "w", encoding="utf-8"), ensure_ascii=False, indent=1)
    for r in results:
        print(
            f"{r['endpoint']:<28} w={r['workers']:<3} n={r['requests']:<4} "
