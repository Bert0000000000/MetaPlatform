...
def main():
    tok = None
    try:
        req = urllib.request.Request(f"{GW}/api/v1/iam/auth/login", method="POST")
        req.add_header("Content-Type", "application/json")
        with urllib.request.urlopen(
            req, data=json.dumps({"username": "admin", "password": "admin123"}).encode(), timeout=30
        ) as r:
    ...
    if tok:
        got = http(
            "GET",
            f"{GW}/api/v1/ont/v2/object-types",
    ...
    if tok:
        ...
        got = http(
            "GET",
            f"{GW}/api/v1/ont/v2/object-types",
            headers={"Authorization": f"Bearer {bad}", "X-Tenant-Id": TENANT},
        )
        rec("jwt-tamper", got == 401, got, "401")
    got = http(
        "GET",
        f"{GW}/api/v1/ont/v2/object-types",
        headers={"Authorization": "Bearer eyJhbGciOiJSUzI1NiJ9.e30.sig", "X-Tenant-Id": TENANT},
    )
    ...
    if tok:
        for payload in ("' OR 1=1 --", "'; DROP TABLE ont_object_type; --"):
            ...
            rec(f"sqli:{payload[:20]}", got in (400, 404, 422), got, "400/404/422 (no 500)")
    ...
    if tok:
        ...
        got = http("POST", f"{GW}/api/v1/marketplace/install", body=big, headers=auth)
    ...
    with open(
        r"D:/Hermes/Workspace/10_Projects/2026-07-02-MetaPlatform/"
        "docs/active/delivery/evidence/SEC-BATTERY-V1.0.json",
        "w",
        encoding="utf-8",
    ) as f:
