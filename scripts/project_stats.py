...
def count_lines(files: list) -> dict:
    ...
    for fp in files:
        ...
        if fp.endswith((".py", ".sh")) or fp.endswith((".yaml", ".yml")):
...
test_py = [
    f
    for f in backend_py
    if "\\tests\\" in f.replace("/", "\\") or "\\test_" in os.path.basename(f).lower()
]
...
other_md = [
    ...
    if "\\docs\\active\\prd\\" not in f.replace("/", "\\")
...
print("-" * 70)
print(
    f"{'后端 Python 代码':<28} {bs['files']:>6} {bs['lines']:>10,} {bs['code']:>10,} {bs['comments']:>7,} {bs['blank']:>7,}"
)
print(
    f"{'├ 应用代码(src)':<28} {bs['files'] - ts['files'] - alembic_s['files']:>6} {(bs['lines'] - ts['lines'] - alembic_s['lines']):>10,} {(bs['code'] - ts['code'] - alembic_s['code']):>10,}"
)
print(
    f"{'├ 测试代码(tests)':<28} {ts['files']:>6} {ts['lines']:>10,} {ts['code']:>10,} {ts['comments']:>7,} {ts['blank']:>7,}"
)
print(
    f"{'└ Alembic migrations':<28} {alembic_s['files']:>6} {alembic_s['lines']:>10,} {alembic_s['code']:>10,} {alembic_s['comments']:>7,} {alembic_s['blank']:>7,}"
)
print()
...
app_py = [
    f
    for f in backend_py
    if "\\tests\\" not in f.replace("/", "\\") and "\\.venv" not in f.replace("/", "\\")
]
