...
import {
  ...
  batchReview,
  commitToOntology,
} from '@/api/dw/extraction';
import type { ExtractionItem, ExtractionType, ExtractionStatus } from '@/api/dw/types';
...
const TYPE_CONFIG: Record<ExtractionType, { label: string; color: TagColor; icon: React.ReactNode }> = {
...
const STATUS_CONFIG: Record<ExtractionStatus, { label: string; color: TagColor }> = {
  pending: { label: '待审核', color: 'grey' },
  approved: { label: '已通过', color: 'green' },
  rejected: { label: '已拒绝', color: 'red' },
  committed: { label: '已提交', color: 'blue' },
};

/** Statistic 自建：label + 大数字（Semi 无 Statistic） */
function StatItem({
  title,
  value,
  color,
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <div style={{ fontSize: 13, color: 'var(--muted-foreground)', marginBottom: 6 }}>{title}</div>
      <div
        style={{
          fontSize: 22,
          fontWeight: 600,
...
export default function ExtractionPanel({ employeeId, documentId }: ExtractionPanelProps) {
  ...
  const handleBatchReview = useCallback(
    async (status: 'approved' | 'rejected') => {
      ...
      try {
        ...
        Toast.success(`已批量${status === 'approved' ? '通过' : '拒绝'} ${selectedIds.length} 项`);
        setItems((prev) =>
          prev.map((i) =>
  ...
  const handleCommit = useCallback(async () => {
    ...
    try {
      ...
      Toast.success(`提交完成：${successCount} 项成功${failCount > 0 ? `，${failCount} 项失败` : ''}`);
      setItems((prev) =>
        prev.map((i) => {
  ...
  const handleExtract = useCallback(async () => {
    ...
    try {
      ...
      Toast.success(`AI 抽取完成：${result.items.length} 项`);
    } catch (error) {
  ...
  const renderItemsTable = (type: ExtractionType) => {
    ...
    const columns: SemiColumns<ExtractionItem> = [
      {
        title: '名称',
        dataIndex: 'name',
        key: 'name',
        ...
        render: (val: number) => (
          <Tooltip content={`${val}%`}>
            <Progress
              percent={val}
              size="small"
              stroke={val >= 90 ? 'var(--semi-color-success)' : val >= 70 ? 'var(--semi-color-warning)' : 'var(--semi-color-danger)'}
            />
          </Tooltip>
        ),
      },
  ...
  return (
    <div>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={4}>
          <Card>
            <StatItem title="概念" value={stats.concepts} icon={<ApartmentOutlined />} />
          </Card>
        </Col>
        <Col span={4}>
          <Card>
            <StatItem title="实体" value={stats.entities} icon={<TeamOutlined />} />
          </Card>
        </Col>
        <Col span={4}>
          <Card>
            <StatItem title="规则" value={stats.rules} icon={<SafetyOutlined />} />
          </Card>
        </Col>
        <Col span={4}>
          <Card>
            <StatItem title="Action" value={stats.actions} icon={<ThunderboltOutlined />} />
          </Card>
        </Col>
        <Col span={4}>
          <Card>
            <StatItem title="已通过" value={stats.approved} color="var(--semi-color-success)" />
          </Card>
        </Col>
        <Col span={4}>
          <Card>
            <StatItem title="已提交" value={stats.committed} color="var(--semi-color-primary)" />
          </Card>
        </Col>
      </Row>
      ...
      >
        {(Object.keys(TYPE_CONFIG) as ExtractionType[]).map((type) => (
          <Tabs.TabPane
            key={type}
            itemKey={type}
            tab={
      ...
      >
        <Banner
          type="warning"
          title={`即将提交 ${approvedItems.filter((i) => i.type === activeTab).length} 项已通过的内容到 Ontology 引擎`}
