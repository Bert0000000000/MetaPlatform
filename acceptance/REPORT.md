...
- **metaplatform-frontend/apps/superai/vite.config.ts** rewritten with 4 Vite proxy entries that map the legacy /v1/copilot/_ frontend surface to real TECH-_ backends, with
  ewrite: functions translating prefixes (no frontend code changes needed):
  - /v1/copilot/auth/_ -> TECH-IAM :8101 rewrite /v1/copilot/auth -> /api/v1/iam/auth`n - /v1/copilot/ontology/_ -> TECH-ONT :8201 rewrite /v1/copilot/ontology -> /api/v1/ont`n  - /v1/copilot/knowledge-bases + /v1/copilot/search -> TECH-RAG :8901  fixed rewrite to /api/v1/rag/knowledge-bases and /api/v1/rag/search`n - /v1/copilot/ (catch-all) -> TECH-AGENT :8511 rewrite /v1/copilot/ -> /api/v1/agent/ (superai/runs/conversations/actions/plans/agents/chat/models/a2a/tasks/skills/memory/...)
